...
- <ProductName /> running locally, with `http://localhost:5173` added to the server-config `cors` section so the web app can call <ProductName /> from the browser. Create the declarative resource `backend/cmd/server/config/resources/server_configs/cors.yaml` and set `server_config.store: composite` in `deployment.yaml` to load it at boot, or apply it without a restart via `PUT /server-config/cors`. See [Get <ProductName />](/docs/next/guides/getting-started/get-thunderid).
