...
`setup.sh` creates the default resources via the in-process `bootstrap` one-shot. It delegates to `start.sh --bootstrap`, which seeds resources through the service layer (no temporary server or security bypass) and runs the bundled consent server for the run. `start.sh` then starts the server with security enabled.
...
This handles the full setup automatically. The script seeds the default resources via the in-process bootstrap, starts the server (with security enabled) loading the sample-app declarative resources, and authenticates as the admin user to run Playwright. Stop any already-running <ProductName /> instance before using this command.
