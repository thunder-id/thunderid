...
const (
	// flowInitiationRedirectOnly indicates the application signs users in through a redirect-based
	// protocol component (currently OAuth 2.0 apps using the authorization_code grant). Such
	// applications must have their flows initiated by that component and may not initiate a new flow
	// via a direct HTTP call.
	flowInitiationRedirectOnly flowInitiationMode = iota
	// flowInitiationAppSecret indicates a backend / server-side application — one that does not sign
	// in by redirect, or an embedded app with no protocol profile at all — that may initiate a flow
	// directly by presenting a valid App Secret.
	flowInitiationAppSecret
)
