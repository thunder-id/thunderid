...
import {useState, useCallback, useMemo, type ReactNode, type SyntheticEvent} from 'react';
...
/**
 * Props passed to a {@link ApplicationEditPageProps.renderFlowsSettings} implementation.
 *
 * @public
 */
export interface FlowsSettingsRenderProps {
...
/**
 * Props passed to a {@link ApplicationEditPageProps.renderIntegrationGuides} implementation.
 *
 * @public
 */
export interface IntegrationGuidesRenderProps {
  application: Application;
 ...
 *
 * `configure-applications` can't depend on `configure-flows` (`configure-flows` already depends on
 * it, for `Application`), so the flows-settings tab and the overview tab's integration guides,
 * both of which need flow data, are supplied by the host through these render props instead of
 * being imported directly.
 *
 * @public
 */
export interface ApplicationEditPageProps {
  renderFlowsSettings: (renderProps: FlowsSettingsRenderProps) => ReactNode;
  renderIntegrationGuides: (renderProps: IntegrationGuidesRenderProps) => ReactNode;
}
...
export default function ApplicationEditPage({renderFlowsSettings, renderIntegrationGuides}: ApplicationEditPageProps) {
  ...
  const handleBack = async () => {
    await navigate(applicationRoutes.applications.list());
  };
    ...
    ? (
        [
          ...
          },
          {
            ...
            panel: renderFlowsSettings({application, editedApp, onFieldChange: handleFieldChange}),
            hidden: isMcpM2mOnly,
          },
          {
            key: 'customization',
  ...
  return (
    <PageContent>
      ...
      {/* Header */}
      <PageTitle>
        <PageTitle.BackButton component={<Link to={applicationRoutes.applications.list()} />}>
          {t('applications:edit.page.back')}
