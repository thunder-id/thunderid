...
func (s *ServiceTestSuite) TestGetResourceDependencies_TypeNotFound_NoBlockingUsages() {
	...
	ets.On("GetEntityType", mock.Anything, entitytype.TypeCategoryUser, "type-1", false).
		Return(nil, &entitytype.ErrorUserTypeNotFound)
...
// A non-not-found ServiceError (e.g. a transient failure) must propagate as an error rather than
// being swallowed as "no dependencies", which would let a blocked deletion through unchecked.
func (s *ServiceTestSuite) TestGetResourceDependencies_OtherServiceError_Propagates() {
	svc, ets := s.newSvcWithEntityType()
	ets.On("GetEntityType", mock.Anything, entitytype.TypeCategoryUser, "type-1", false).
