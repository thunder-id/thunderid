...
The JWKS cache time-to-live applies to signing keys fetched from this endpoint and is configured at `server.security.jwks_cache_ttl`, not inside the `trusted_issuer` block. See [Security Configuration](../../getting-started/configuration#security-configuration) for details.
...
- [Security Configuration](../../getting-started/configuration#security-configuration), configure JWKS cache TTL and other security settings.
