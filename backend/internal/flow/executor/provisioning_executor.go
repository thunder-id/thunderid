...
func (p *provisioningExecutor) getTargetEntityRef(ctx *providers.NodeContext,
	...
	ouID := p.getOUID(ctx)
	entityType := p.getEntityType(ctx, category)
...
// getEntityType retrieves the entity type to provision into from runtime data, where each category
// carries its type under its own runtime key. An empty result leaves the target to be resolved from
// the application's configuration.
func (p *provisioningExecutor) getEntityType(ctx *providers.NodeContext,
	category entitytype.TypeCategory) string {
	if val, ok := ctx.RuntimeData[runtimeEntityTypeKey(category)]; ok && val != "" {
		return val
	}
...
// runtimeEntityTypeKey returns the runtime data key that carries the entity type for the category.
func runtimeEntityTypeKey(category entitytype.TypeCategory) string {
	if category == entitytype.TypeCategoryAgent {
		return agentTypeKey
	}
	return userTypeKey
