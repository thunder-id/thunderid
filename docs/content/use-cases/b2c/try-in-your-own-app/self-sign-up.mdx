...
Build a `REGISTRATION` flow using the Flow Designer or the flows API. See [Build a Flow](../../../../guides/flows/build-a-flow).
...
To automatically assign a default role to every self-registered user, create the role under **Roles** → **Add Role**. Then set `properties.assignRole` to that role's ID on the `ProvisioningExecutor` node in your flow. See [Authorization](../../../../key-concepts/authorization).
...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| React       | [React Quickstart](../../../../getting-started/connect-your-application/react), `<SignUpButton />` component                       |
