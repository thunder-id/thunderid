...
// two known gaps in this server's SCIM support (no Users PATCH, no ETag) so
// a regression here is caught immediately.
...
// scimPaginationMaxPageSize mirrors the PaginationMaxPageSize set in
// scimconfig.FromServerRuntime (backend/internal/scim/config/config.go). Update here
...
func (ts *SCIMDiscoveryTestSuite) TestServiceProviderConfig() {
	...
	ts.False(cfg.ETag.Supported,
		"ETag should report unsupported: no persisted version column backs optimistic concurrency control")
...
// ServiceProviderConfig, requires authentication: no
