...
| `server.security.management_token` | `""` (empty) | Pre-shared token that authenticates calls to this deployment's import API (`/import`, `/import/**`) and variable store (`/variables`, `/variables/**`, `/secrets`, `/secrets/**`), so a deployment pipeline can push configuration without first obtaining OAuth client credentials. Presented as an ordinary bearer token: `Authorization: Bearer <token>`. It authenticates as a principal holding the **root system permission**, and is trusted **only on those paths**; every other API stays behind OAuth. A bearer value that is not this token is passed to the normal OAuth check, so tokens keep working on the same paths. Empty **disables it**, which is the default. Because it is a single long-lived value it cannot be scoped per caller or revoked without a restart, so treat it as a deployment credential, give it to as few callers as possible, and prefer an OAuth client wherever one can be obtained |
