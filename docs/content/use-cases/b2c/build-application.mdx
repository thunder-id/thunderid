...
- Which user types can sign up through it, so the customer-facing app onboards `Customer` users
  and not `Staff`.
- Which flow runs for each journey it offers, from the bundled `default-flow` to the ones you build, including the
  one that signs a customer out.
...
| Post-Logout Redirect URIs | `{{WayFinderSampleUrl}}`  |
| Allowed grants            | `authorization_code`      |
| PKCE                      | Required                  |
| Allowed user types        | `Customer`                |

The page a customer returns to after signing out must be on the post-logout list, or <ProductName /> rejects the
request.
...
In **Applications** → **WAYFINDER** → **Flows**, select the registration flow you built from the **Registration Flow** dropdown, the recovery flow from the **Recovery Flow** dropdown, and the sign-out flow from the **Sign Out Flow** dropdown, then save the application.
