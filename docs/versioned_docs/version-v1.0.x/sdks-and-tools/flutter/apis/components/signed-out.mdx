...
`SignedOut` is a guard widget that renders its `child` only when the user is **not** authenticated. When the user is signed in it renders `fallback` instead (or nothing, if no fallback is provided).
...
Using `SignedOut` and `SignedIn` together in a root router:
