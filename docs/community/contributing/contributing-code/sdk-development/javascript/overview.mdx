...
| `@thunderid/better-auth` | Better Auth provider helper |

`@thunderid/better-auth` stands apart from the rest: it has no dependency on `@thunderid/javascript` and no protocol logic of its own, it only returns a typed OAuth config for [Better Auth](https://better-auth.com)'s Generic OAuth plugin. The other packages build on each other rather than standing alone. Each arrow points from a package to the ones built on top of it:
