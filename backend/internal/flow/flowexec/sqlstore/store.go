...
// Package sqlstore provides the relational-database-backed flow context store.
// It is isolated from the flowexec package so that consumers needing only the
...
// flowStore implements flowexec.FlowStoreInterface for managing flow contexts.
type flowStore struct {
	dbProvider   provider.DBProviderInterface
	deploymentID string
}

// NewStore creates a new database-backed flow context store.
func NewStore(dbProvider provider.DBProviderInterface, deploymentID string) flowexec.FlowStoreInterface {
	return &flowStore{
...
func (s *flowStore) StoreFlowContext(ctx context.Context, dbModel flowexec.FlowContextDB, expirySeconds int64) error {
...
func (s *flowStore) GetFlowContext(ctx context.Context, executionID string) (*flowexec.FlowContextDB, error) {
	var result *flowexec.FlowContextDB
...
func (s *flowStore) UpdateFlowContext(ctx context.Context, dbModel flowexec.FlowContextDB) error {
	return withRuntimeDBClientContext(ctx, s.dbProvider, func(dbClient provider.DBClientInterface) error {
...
// buildFlowContextFromResultRow builds a flowexec.FlowContextDB from a database result row.
func (s *flowStore) buildFlowContextFromResultRow(row map[string]interface{}) (*flowexec.FlowContextDB, error) {
