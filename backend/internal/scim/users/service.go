...
type SCIMUsersServiceInterface interface {
	...
	ValidateFilterSchemaAttribute(ctx context.Context, prefix, attr string) *tidcommon.ServiceError
}
...
func (s *scimUsersService) ListUsers(ctx context.Context, startIndex, count int,
	...
	isCoreType := s.coreUserTypeMatcher(ctx)
	...
	for _, u := range listResp.Users {
		...
		if !ok {
			...
			rawProps, typeID, svcErr = s.getSchemaProps(ctx, u.Type)
			if svcErr != nil {
				s.logger.Warn(ctx, "SCIM ListUsers: returning user without attributes, unresolvable user type",
					log.MaskedString(log.LoggerKeyUserID, u.ID), log.String("userType", u.Type))
...
func (s *scimUsersService) GetUser(
	...
	includeCoreAttrs := s.coreUserTypeMatcher(ctx)(u.Type, typeID)
...
func (s *scimUsersService) CreateUser(
	...
	extensionURN := scim.BuildSchemaURN(s.cfg.SchemaURNPrefix, created.Type)
...
func (s *scimUsersService) ReplaceUser(
	...
	extensionURN := scim.BuildSchemaURN(s.cfg.SchemaURNPrefix, result.Type)
	// et was already fetched above for processInboundPayload, so its schema is reused here
	// rather than re-fetching the entity type a second time.
	rawProps, parseErr := scim.ParseRawProperties(et.Schema)
	if parseErr != nil {
...
// ValidateFilterSchemaAttribute checks a schema-URN-qualified filter path the same way
// "attributes" paths are checked: the URN must name the core schema, the enterprise schema or
// a registered user type, and attr must belong to that schema.
func (s *scimUsersService) ValidateFilterSchemaAttribute(
	ctx context.Context, prefix, attr string,
) *tidcommon.ServiceError {
	urn := strings.TrimSuffix(prefix, ":")
	path := urn + ":" + attr
	root := attr
	...
	switch {
	case strings.EqualFold(urn, scim.SCIMCoreUserSchemaURN):
		if !isCoreSCIMFilterAttr(attr) {
			return scim.NewUnrecognizedAttributeError(path)
		}
		return nil
	case strings.EqualFold(urn, scim.SCIMEnterpriseUserSchemaURN):
		if !isEnterpriseSCIMAttrPath(root) {
			return scim.NewUnrecognizedAttributeError(path)
		}
		return nil
	}
...
func (s *scimUsersService) validateAttributePath(ctx context.Context, attr string) *tidcommon.ServiceError {
	...
	if _, ok := scim.ParseUserTypeFromSchemaURN(s.cfg.SchemaURNPrefix, attr); ok {
		return nil
	}
...
func (s *scimUsersService) coreUserTypeMatcher(ctx context.Context) func(typeName, typeID string) bool {
	...
	return func(typeName, _ string) bool {
		...
		return coreTypeName != "" && strings.EqualFold(typeName, coreTypeName)
	}
}
