...
Every agent that mints or receives a delegated token needs the token exchange grant.

- **Console:** on each agent's **Advanced** tab, with Delegated mode on, add the **Token Exchange** grant.
