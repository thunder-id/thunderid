...
func TestGetDynamicMatcher_BadEntriesReturnNil(t *testing.T) {
	...
	// An uncompilable entry (invalid regex) fails closed: the matcher is dropped so the middleware denies.
...
func TestGetDynamicMatcher_WrongTypeReturnsNil(t *testing.T) {
	...
	// A merged value that is not an OriginConfig fails closed.
...
func TestGetDynamicMatcher_ReadErrorReturnsNil(t *testing.T) {
	...
	// A read error fails closed even though a good matcher had compiled before.
	fail = true
	assert.Nil(t, GetDynamicMatcher(context.Background()))
}
