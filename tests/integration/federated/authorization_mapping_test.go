...
/*
AuthorizationRuleMapping integration tests.

These exercise the connection's authorizationMapping.rules section end to end: a mock IdP returns claims,
...
counterpart to the token-exchange AuthorizationRuleMapping scenarios in
...
// authzMapping builds a single-claim AuthorizationRuleMapping configuration, alongside the attribute
...
// authzMappingWithRules builds a single-claim AuthorizationRuleMapping configuration from explicit rules,
...
// Mapped access combines with a real, non-mapped local group membership too, not just direct role
// assignments (see TestAuthzMapping_DirectAssignmentPlusMappedRoleCombine for the role-side version):
// an entity is a genuine member of one group locally, and a mapping resolves to a different group on
...
func (s *FederatedMappingSuite) TestAuthzMapping_LocalGroupPlusMappedGroupCombine() {
	...
	directGroupID, err := testutils.CreateGroup(testutils.Group{
		...
		OUID:    s.ouID,
		Members: []testutils.Member{{Id: entity.ID, Type: "user"}},
	})
	s.Require().NoError(err, "failed to create the direct-membership group")
	defer func() {
		if err := testutils.DeleteGroup(directGroupID); err != nil {
			s.T().Logf("failed to delete the direct-membership group: %v", err)
		}
	}()

	directGroupRoleID, err := testutils.CreateRole(testutils.Role{
		Name: "Federated Direct Group Deleter Role " + s.nextSubject(),
		OUID: s.ouID,
		Permissions: []testutils.ResourcePermissions{
			{ResourceServerID: s.authzResourceServerID, Permissions: []string{"delete"}},
		},
		Assignments: []testutils.Assignment{{ID: directGroupID, Type: "group"}},
	})
	s.Require().NoError(err, "failed to create the direct group's role")
	defer func() {
		if err := testutils.DeleteRole(directGroupRoleID); err != nil {
			s.T().Logf("failed to delete the direct group's role: %v", err)
		}
	}()
	...
	user.Custom["groups"] = []interface{}{"platform-editors"}

	authorized := s.authorizeFederated(config, user, "write delete", "federated-authz-mapping-api")
	s.ElementsMatch([]string{"write", "delete"}, authorized,
		"the entity's real group membership and its mapped group should combine")
}

// A group reachable through both a real local membership and a mapping at the same time (an entity
// already belongs to a group, and the mapping also resolves to that same group on this login) must
...
func (s *FederatedMappingSuite) TestAuthzMapping_OverlappingLocalAndMappedGroupDeduped() {
	...
	// membership and the mapping at once.
	overlapGroupID, err := testutils.CreateGroup(testutils.Group{
		Name:    "Federated Overlap Group " + s.nextSubject(),
		OUID:    s.ouID,
		Members: []testutils.Member{{Id: entity.ID, Type: "user"}},
	})
	s.Require().NoError(err, "failed to create the overlap group")
	defer func() {
		if err := testutils.DeleteGroup(overlapGroupID); err != nil {
			s.T().Logf("failed to delete the overlap group: %v", err)
		}
	}()

	overlapRoleID, err := testutils.CreateRole(testutils.Role{
		Name: "Federated Overlap Group Role " + s.nextSubject(),
		OUID: s.ouID,
		Permissions: []testutils.ResourcePermissions{
			{ResourceServerID: s.authzResourceServerID, Permissions: []string{"write"}},
		},
		Assignments: []testutils.Assignment{{ID: overlapGroupID, Type: "group"}},
	})
	s.Require().NoError(err, "failed to create the overlap group's role")
	defer func() {
		if err := testutils.DeleteRole(overlapRoleID); err != nil {
			s.T().Logf("failed to delete the overlap group's role: %v", err)
		}
	}()

	config := authzMapping("groups", "", map[string][]testutils.AuthorizationTarget{
		"platform-editors": {groupTarget(overlapGroupID)},
	})
	user.Custom["groups"] = []interface{}{"platform-editors"}

	authorized := s.authorizeFederated(config, user, "write", "federated-authz-mapping-api")
	s.ElementsMatch([]string{"write"}, authorized,
		"a group reachable via both local membership and mapping must resolve once, not fail or duplicate")
}

// The counterpart to TestAuthzMapping_ProvisioningSeedsRolesAndGroupsFromMapping: without
// seedRolesFromMapping/seedGroupsFromMapping opted in on the provisioning node (the default, used by
// every other scenario in this file via authorizeFederated), a mapped role or group still authorizes
// the login dynamically, but nothing is persisted — the entity gets no durable assignment or
// membership from it.
func (s *FederatedMappingSuite) TestAuthzMapping_WithoutSeedingFlagsMappingIsNotPersisted() {
	user := s.baseUser(s.nextSubject())
	user.Custom["groups"] = []interface{}{"platform-admins", "platform-editors"}
	config := authzMapping("groups", "", map[string][]testutils.AuthorizationTarget{
		"platform-admins":  {roleTarget(s.authzMappedRoleID)},
	...
	s.Require().NoError(err, "failed to read the mapped role's assignments")
	for _, a := range roleAssignments {
		s.NotEqual(entity.ID, a.ID, "without seedRolesFromMapping, no durable role assignment should be created")
	}

	groupMembers, err := testutils.GetGroupMembers(s.authzMappedGroupID)
	s.Require().NoError(err, "failed to read the mapped group's members")
	for _, m := range groupMembers {
		s.NotEqual(entity.ID, m.ID, "without seedGroupsFromMapping, no durable group membership should be created")
	}
}
