...
interface PDPIntegrationDiagramProps {
  ...
  /** Optional brand background colour for the gateway box (e.g. '#041c36'). */
  gatewayBgColor?: string;
  /** Whether to show an authorization adapter between the gateway and PDP. */
  showAdapter?: boolean;
}
  ...
  return (
    <div className="sts-diagram">
      ...
      >
        ...
        {/* ThunderID AuthZEN PDP */}
        <g className="sts-diagram__idp" transform="translate(780,210)">
          <rect width="220" height="130" rx="10" />
          <g transform="translate(92,28) scale(0.174)">
            <path d="M55.4763 26.4391L58.8866 0H0V26.4391H55.4763Z" className="sts-diagram__idp-logo-dark" />
            <path d="M39.8438 147.407L49.5455 72.2839H4.9909e-05V256.743H60.5602L80.048 147.407H39.8438Z" className="sts-diagram__idp-logo-accent" />
            <path d="M192.42 59.361C182.782 40.2307 168.929 25.5705 150.903 15.3381C145.501 12.2662 139.761 9.6605 133.703 7.5208L115.401 103.702H159.757L76.2987 256.743H83.3735C109.449 256.743 131.69 251.574 150.14 241.236C168.569 230.897 182.634 216.131 192.356 196.959C202.058 177.765 206.909 154.8 206.909 128.043C206.909 101.286 202.079 78.5123 192.441 59.3821L192.42 59.361Z" className="sts-diagram__idp-logo-accent" />
          </g>
          <text x="110" y="92" textAnchor="middle" className="sts-diagram__box-title sts-diagram__idp-name">
            {productName}
          </text>
          <text x="110" y="110" textAnchor="middle" className="sts-diagram__box-sub">
            AuthZEN PDP
          </text>
        </g>
        ...
        {showAdapter && (
          <g className="sts-diagram__adapter" transform="translate(410,240)">
            <rect width="200" height="64" rx="10" />
            <text x="100" y="38" textAnchor="middle" className="sts-diagram__box-title">
        ...
        <g className="sts-diagram__edges">
          {/* ① Application → API Gateway */}
          <line x1="220" y1="104" x2="419" y2="104" markerEnd="url(#pdp-arrow)" />
          <text x="320" y="92" textAnchor="middle" className="sts-diagram__edge-label">
            <tspan className="sts-diagram__edge-num">①</tspan> API request + Bearer token
          </text>
          ...
          {showAdapter ? (
            <>
              {/* ② API Gateway → Authorization Adapter */}
              <line x1="480" y1="144" x2="480" y2="239" markerEnd="url(#pdp-arrow)" />
              <text x="466" y="192" textAnchor="end" className="sts-diagram__edge-label">
                <tspan className="sts-diagram__edge-num">②</tspan> Call adapter
              </text>

              {/* ③ Authorization Adapter → ThunderID: AuthZEN evaluation request */}
              <line x1="610" y1="262" x2="779" y2="262" markerEnd="url(#pdp-arrow)" />
              <text x="695" y="246" textAnchor="middle" className="sts-diagram__edge-label">
                <tspan className="sts-diagram__edge-num">③</tspan> AuthZEN request
              </text>

              {/* ④ ThunderID → Authorization Adapter: allow/deny decision */}
              <line x1="780" y1="292" x2="611" y2="292" markerEnd="url(#pdp-arrow)" />
              <text x="695" y="316" textAnchor="middle" className="sts-diagram__edge-label">
                <tspan className="sts-diagram__edge-num">④</tspan> Allow / deny
              </text>

              {/* ⑤ Authorization Adapter → API Gateway: return decision */}
              <line x1="540" y1="240" x2="540" y2="145" markerEnd="url(#pdp-arrow)" />
              <text x="554" y="192" className="sts-diagram__edge-label">
                <tspan className="sts-diagram__edge-num">⑤</tspan> Decision
              </text>
            </>
          ) : (
            <>
              {/* ② API Gateway → ThunderID: AuthZEN evaluation request */}
              <polyline points="510,144 510,250 779,250" markerEnd="url(#pdp-arrow)" />
              <text x="640" y="238" textAnchor="middle" className="sts-diagram__edge-label">
                <tspan className="sts-diagram__edge-num">②</tspan> AuthZEN request
              </text>

              {/* ③ ThunderID → API Gateway: allow/deny decision */}
              <polyline points="780,304 540,304 540,145" markerEnd="url(#pdp-arrow)" />
              <text x="660" y="294" textAnchor="middle" className="sts-diagram__edge-label">
                <tspan className="sts-diagram__edge-num">③</tspan> Allow / deny
              </text>
            </>
          )}
          ...
          {/* ⑥ API Gateway → Protected Resource Server */}
          <line x1="600" y1="104" x2="779" y2="104" markerEnd="url(#pdp-arrow)" />
          <text x="690" y="92" textAnchor="middle" className="sts-diagram__edge-label">
            <tspan className="sts-diagram__edge-num">{showAdapter ? '⑥' : '④'}</tspan> Forward if allowed
          </text>

          {/* ⑦ API Gateway → Application: reject denied request */}
          <line x1="420" y1="132" x2="221" y2="132" markerEnd="url(#pdp-arrow)" />
          <text x="320" y="154" textAnchor="middle" className="sts-diagram__edge-label">
            <tspan className="sts-diagram__edge-num">{showAdapter ? '⑦' : '⑤'}</tspan> 403 if denied
          </text>
        </g>
      </svg>
    </div>
...
export function EnvoyPDPIntegrationDiagram() {
  return (
    <PDPIntegrationDiagram
      gatewayName="Envoy"
      gatewayLogo={<EnvoyLogo color="#fff" compact />}
