...
import {OrganizationDetails, updateOrganization, createPatchOperations} from '@thunderid/node';
import {BaseOrganizationProfile, BaseOrganizationProfileProps, useTranslation} from '@thunderid/react';
...
const OrganizationProfile: FC<OrganizationProfileProps> = ({
  ...
  const fetchOrganization = async (): Promise<void> => {
    ...
    try {
      ...
      const result: {data?: {organization?: OrganizationDetails}; error: string | null; success: boolean} =
        await getOrganizationAction(organizationId, (await getSessionId())!);
  ...
  const handleOrganizationUpdate = async (payload: any): Promise<void> => {
    ...
    try {
      ...
      const operations: {operation: 'REPLACE' | 'REMOVE'; path: string; value?: any}[] = createPatchOperations(payload);
