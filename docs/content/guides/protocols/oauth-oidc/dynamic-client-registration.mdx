...
| `id_token_encrypted_response_alg` | No | Key-management algorithm for ID token encryption. Required when `id_token_encrypted_response_enc` is set. Supported values: `RSA-OAEP`, `RSA-OAEP-256`. |
...
| `400` | `invalid_client_metadata` | `id_token_encrypted_response_enc` is set without `id_token_encrypted_response_alg`. |
