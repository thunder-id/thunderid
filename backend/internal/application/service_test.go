...
func (suite *ServiceTestSuite) TestGetResourceUsages_Success() {
	...
	ep.On("GetEntitiesByIDs", []string{"app-1", "app-2"}).Return([]entityprovider.Entity{
		{ID: "app-1", Category: entityprovider.EntityCategoryApp, SystemAttributes: sysAttrs1},
		{ID: "app-2", Category: entityprovider.EntityCategoryApp, SystemAttributes: sysAttrs2},
	}, (*entityprovider.EntityProviderError)(nil))

	result, err := service.GetResourceUsages(context.Background(), usage.ResourceTypeTheme, "theme-1")
	assert.NoError(suite.T(), err)
	require.Len(suite.T(), result, 2)
	assert.Equal(suite.T(), usage.ResourceTypeApplication, result[0].ResourceType)
...
func (suite *ServiceTestSuite) TestGetResourceUsages_FiltersOutNonAppEntities() {
	...
	ep := resetEntityProviderMethod(service, "GetEntitiesByIDs")
	ep.On("GetEntitiesByIDs", []string{"app-1", "agent-1"}).Return([]entityprovider.Entity{
		{ID: "app-1", Category: entityprovider.EntityCategoryApp, SystemAttributes: sysAttrs},
		{ID: "agent-1", Category: entityprovider.EntityCategoryAgent},
	}, (*entityprovider.EntityProviderError)(nil))
