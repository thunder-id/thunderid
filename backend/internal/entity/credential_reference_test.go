...
// serveHashedSecret stands up a secret provider holding one hash backed secret.
func serveHashedSecret(t *testing.T, name string, hash cryptolib.Credential) {
	t.Helper()
...
func TestCredentialReference_ResolvesAPromotedCredentialFromTheSecretProvider(t *testing.T) {
	...
	serveHashedSecret(t, "MY_APP_CLIENT_SECRET", hash)
...
func TestCredentialReference_RejectsAnUnresolvableReference(t *testing.T) {
	serveHashedSecret(t, "SOMETHING_ELSE", hashOf(t, "x"))
