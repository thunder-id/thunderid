...
import {useEmailProviders, type ConnectionInstance} from '@thunderid/configure-connections';
...
function EmailProperties({resource, onChange}: CommonResourcePropertiesPropsInterface): ReactNode {
  ...
  const {data: notificationSenders, isLoading: isLoadingSenders} = useEmailProviders();
    ...
    <Stack gap={2}>
      ...
      <div>
        ...
        >
          ...
          {notificationSenders?.map((sender: ConnectionInstance) => (
