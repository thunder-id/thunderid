...
const BaseSignUp: Component = defineComponent({
  ...
  setup(props: any, {slots}: SetupContext): () => VNode | null {
    ...
    const currentFlow: Ref<EmbeddedSignUpFlowResponse | null> = ref(null);
    ...
    const handleRedirectionIfNeeded = (response: any): boolean => {
      if ((response as any)?.type !== 'REDIRECTION' || !response?.data?.redirectURL) {
        return false;
      }
      ...
      const processOAuthCode = async (code: string, state: string): Promise<void> => {
        const payload: any = {
          ...(currentFlow.value && {executionId: (currentFlow.value as any).executionId || (currentFlow.value as any).flowId}),
          action: '',
          inputs: {code, state},
        };
    ...
    const handleSubmit = async (
      ...
      try {
        ...
        const payload: any = {
          executionId: (currentFlow.value as any).executionId || (currentFlow.value as any).flowId,
        ...
        if (response.flowStatus === EmbeddedSignUpFlowStatus.Incomplete) {
          ...
          if (response.data?.additionalData?.['passkeyCreationOptions']) {
            ...
            const effectiveExecutionId: string | undefined = response.executionId || (currentFlow.value as any)?.executionId;
    ...
    watch(
      ...
      async (state: PasskeyState) => {
        if (!state.isActive || !state.creationOptions || !state.executionId) return;
        ...
        try {
          ...
          passkeyState.value = {actionId: null, creationOptions: null, error: null, executionId: null, isActive: false};
        } catch (error: unknown) {
