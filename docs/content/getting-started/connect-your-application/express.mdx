...
<CodeGroup>
  ...
  <CodeBlock lang="bash" label="Deno">
    ...
    cd my-express-app
    deno init
    deno add npm:express npm:cookie-parser
  </CodeBlock>
</CodeGroup>
...
:::note Using Deno
The `deno init` scaffold above does not create a `package.json` with `"type": "commonjs"`, so Deno treats a `.js` file as an ES module and the `require()` calls above fail with `require is not defined`. Save the file as `index.mjs` instead, using `import` in place of `require`:
...
Run it with `deno run -A index.mjs` in the next step, instead of `index.js`.
:::
...
<NextSteps>
  ...
  <NextStepsCard title="Express SDK Reference" href="../../../sdks-and-tools/express/overview" description="Explore the full API reference for the Express SDK." cta="API Reference" />
</NextSteps>
