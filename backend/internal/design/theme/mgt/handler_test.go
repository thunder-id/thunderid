...
type mockThemeService struct {
	...
	getThemeUsagesFunc func(id string, limit, offset int) (*usage.UsagesResponse, *serviceerror.ServiceError)
}
...
func (m *mockThemeService) SetUsageRegistry(_ usage.Registry) {}

func (m *mockThemeService) GetThemeUsages(
	_ context.Context, id string, limit, offset int) (*usage.UsagesResponse, *serviceerror.ServiceError) {
...
func (suite *ThemeHandlerTestSuite) TestHandleThemeUsagesGetRequest_Success() {
	...
	assert.Equal(suite.T(), usage.ResourceTypeApplication, response.Usages[0].ResourceType)
	assert.Equal(suite.T(), usage.BehaviorFallback, response.Usages[0].BehaviorOnDelete)
}
