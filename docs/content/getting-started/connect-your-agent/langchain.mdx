...
<CodeGroup>
  ...
  <CodeBlock lang="bash" label="Yarn">
    yarn init -y
    yarn add langchain @langchain/core @langchain/google-genai dotenv open zod
    yarn add -D @types/node tsx typescript
  </CodeBlock>

  <CodeBlock lang="bash" label="Bun">
    bun init -y
    bun add langchain @langchain/core @langchain/google-genai dotenv open zod
    bun add -d @types/node tsx typescript
  </CodeBlock>

  <CodeBlock lang="bash" label="Deno">
    deno init
    deno add npm:langchain npm:@langchain/core npm:@langchain/google-genai npm:dotenv npm:open npm:zod
    deno add -D npm:@types/node npm:tsx npm:typescript
  </CodeBlock>
</CodeGroup>
...
:::note Using Deno
Skip this step. Deno treats a `.ts` file as an ES module by default and does not need a
`package.json` at all.
:::
...
Using Deno? Run `deno run -A agent_own_token.ts` instead, since Deno runs TypeScript natively and does not need `tsx`.
...
Using Deno? Run `deno run -A agent_obo.ts` instead, since Deno runs TypeScript natively and does not need `tsx`.
