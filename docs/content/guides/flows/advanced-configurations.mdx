...
**Prerequisites:**
- `email` (required), the recipient email address must be resolvable before this executor runs. It is not a configurable node input. The executor resolves it in priority order: forwarded data from prior executors (e.g., Generate Invite, Magic Link), user inputs, then the user store, looked up via `userID`. The executor fails if none of these resolve to a non-empty address.
