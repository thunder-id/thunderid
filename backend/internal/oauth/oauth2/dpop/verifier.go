...
func (v *verifier) Verify(ctx context.Context, params VerifyParams) (*ProofResult, error) {
	...
	if err := v.verifyProofSignature(params.Proof, jwk); err != nil {
		return nil, err
	}
...
func (v *verifier) verifyProofSignature(proof string, jwk map[string]any) error {
	pubKey, err := jws.JWKToPublicKey(jwk)
	if err != nil {
		return fmt.Errorf("%w: %s", ErrInvalidProof, err.Error())
	}

	validationErr := v.jwtService.VerifyJWTSignatureWithPublicKey(proof, pubKey)
	if validationErr != nil {
		return fmt.Errorf("%w: %s", ErrInvalidProof, validationErr)
	}
	return nil
}
