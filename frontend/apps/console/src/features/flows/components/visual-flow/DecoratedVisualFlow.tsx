  ...
  const handleDuplicateNodes = useCallback(
    (nodesToCopy: Node[]): void => {
      ...
      setNodes((currentNodes: Node[]) => {
        // Seeded with every node AND nested component id, and shared across the
        // copies, so no generated id can collide with anything on the canvas.
        const takenIds = collectCanvasIds(currentNodes);
        const copies = duplicable.map((node: Node) => duplicateFlowNode(node, takenIds));
  ...
  const handleContextAddStep = useCallback(
    (stepResource: Resource): void => {
      ...
      const generatedStep: Step = onStepLoad({
        ...
        deletable: true,
        // Allocated against the real graph so the id cannot collide with a
        // node or nested component, including stack members hidden in compact mode.
        id: generateUniqueResourceId(clonedResource.type.toLowerCase(), collectCanvasIds(sourceNodes ?? getNodes())),
        position,
      } as Step);
      setNodes((prevNodes: Node[]) => [...prevNodes, generatedStep]);
      onResourceDropOnCanvas(generatedStep, '');
    },
    [contextMenuTarget, screenToFlowPosition, onStepLoad, setNodes, onResourceDropOnCanvas, sourceNodes, getNodes],
  );
  ...
  return (
    ...
    >
      ...
      <CanvasContextMenu
        ...
        // Synthetic compact-mode stacks are display-only; the preview cannot
        // meaningfully start from them.
        canPreview={Boolean(contextMenuNode) && contextMenuNode?.type !== EXECUTION_STACK_NODE_TYPE}
