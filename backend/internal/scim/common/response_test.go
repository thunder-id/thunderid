...
// ResponseTestSuite groups the tests in response_test.go.
type ResponseTestSuite struct {
	suite.Suite
}

// TestResponseTestSuite runs ResponseTestSuite.
...
func (suite *ResponseTestSuite) TestWriteSCIMSuccessResponse_EncodeError() {
	t := suite.T()
	rr := httptest.NewRecorder()
	WriteSCIMSuccessResponse(
		context.Background(), rr, http.StatusOK, marshalErrorStruct{}, *log.GetLogger())
	require.Equal(t, http.StatusInternalServerError, rr.Code)
}

// TestWriteSCIMErrorResponse_EmptySchemas tests Write SCIM Error Response for Empty Schemas.
func (suite *ResponseTestSuite) TestWriteSCIMErrorResponse_EmptySchemas() {
	t := suite.T()
	rr := httptest.NewRecorder()
...
func (suite *ResponseTestSuite) TestMapSCIMError_UncoveredCodes() {
	t := suite.T()
	...
	status9, scimType9 := mapSCIMError(&ErrorMutabilityViolation)
	require.Equal(t, http.StatusBadRequest, status9)
	require.Equal(t, ScimErrorTypeMutability, scimType9)
	...
	// RFC 7644 §3.5.2.2: a remove without a path must report scimType "noTarget".
	status8, scimType8 := mapSCIMError(&ErrorNoTarget)
	require.Equal(t, http.StatusBadRequest, status8)
	require.Equal(t, ScimErrorTypeNoTarget, scimType8)
}
...
func (suite *ResponseTestSuite) TestMapSCIMError_CoversAllDefinedErrors() {
	t := suite.T()
