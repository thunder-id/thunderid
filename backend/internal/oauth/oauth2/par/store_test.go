...
// backend-failure branches of Store/Consume, which a real store cannot be made to fail on demand.
...
func (f *erroringStoreProvider) Take(
...
func (s *StoreTestSuite) TestStore_PutError() {
	store := &parRequestStore{storeProvider: &erroringStoreProvider{putErr: errors.New("put failed")}}
...
func (s *StoreTestSuite) TestConsume_TakeError() {
	store := &parRequestStore{storeProvider: &erroringStoreProvider{takeErr: errors.New("take failed")}}
