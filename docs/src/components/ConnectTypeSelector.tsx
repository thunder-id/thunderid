...
import {Bot, Check, ChevronDown, MonitorSmartphone, Server} from 'lucide-react';
...
export default function ConnectTypeSelector(): React.ReactElement {
  ...
  useEffect(() => {
    const stored = (localStorage.getItem(STORAGE_KEY) as ConnectType | null) ?? 'app';
    setSelected(stored);
    applyType(stored);
  }, []);
