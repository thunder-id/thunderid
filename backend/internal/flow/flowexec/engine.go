...
func (fe *flowEngine) Execute(ctx *EngineContext) (FlowStep, *serviceerror.ServiceError) {
	...
	if _, svcErr := fe.runInterceptors(
		common.InterceptorModePreRequest, ctx, nil, &flowStep); svcErr != nil {
	...
	for currentNode != nil {
		...
		if continueExec, svcErr := fe.runInterceptors(
			common.InterceptorModePreNode, ctx, currentNode, &flowStep); svcErr != nil {
			publishFlowFailedEvent(ctx, svcErr, flowStartTime, time.Now().UnixMilli(), fe.observabilitySvc)
			return flowStep, svcErr
		} else if !continueExec {
			// INCOMPLETE: run POST_REQUEST interceptors for cleanup, then return.
			if _, postSvcErr := fe.runInterceptors(
				common.InterceptorModePostRequest, ctx, nil, &flowStep); postSvcErr != nil {
				publishFlowFailedEvent(ctx, postSvcErr, flowStartTime, time.Now().UnixMilli(), fe.observabilitySvc)
				return flowStep, postSvcErr
			}
			return flowStep, nil
		}
		...
		if !continueExecution {
			// Run POST_REQUEST interceptors for incomplete flows.
			if _, svcErr := fe.runInterceptors(
				common.InterceptorModePostRequest, ctx, nil, &flowStep); svcErr != nil {
				publishFlowFailedEvent(ctx, svcErr, flowStartTime, time.Now().UnixMilli(), fe.observabilitySvc)
				return flowStep, svcErr
			}
		...
		if continueExec, svcErr := fe.runInterceptors(
			common.InterceptorModePostNode, ctx, currentNode, &flowStep); svcErr != nil {
			publishFlowFailedEvent(ctx, svcErr, flowStartTime, time.Now().UnixMilli(), fe.observabilitySvc)
			return flowStep, svcErr
		} else if !continueExec {
			// INCOMPLETE: run POST_REQUEST interceptors for cleanup, then return.
...
func (fe *flowEngine) runInterceptors(
	...
	// Get interceptors from the graph
	if ctx.Graph == nil {
		return true, nil
	}

	interceptors := ctx.Graph.GetInterceptors()
