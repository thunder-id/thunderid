...
func TestNewAuthZENPDPAppliesDefaultTimeout(t *testing.T) {
	...
	require.True(t, ok)
	require.Equal(t, time.Duration(config.GetServerRuntime().Config.AuthZENPDP.TimeoutMS)*time.Millisecond, pdp.timeout)
}
