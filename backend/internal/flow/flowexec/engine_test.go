...
func (s *EngineTestSuite) TestFlowStartedEvent_ReportsPrincipalAndCorrelation() {
	mockObs, captured := setupCapturingObservability(s.T())
	...
	s.Equal("flow-exec-1", evt.TraceID)

	assertEventData(s.T(), evt, map[string]interface{}{
...
func (s *EngineTestSuite) TestFlowStartedEvent_SubjectComesFromAuthUser() {
	mockObs, captured := setupCapturingObservability(s.T())
	defer config.ResetServerRuntime()
	...
	assertEventData(s.T(), (*captured)[0], map[string]interface{}{
...
func (s *EngineTestSuite) TestFlowStartedEvent_AgentSubjectReportsAgentCategory() {
	mockObs, captured := setupCapturingObservability(s.T())
	defer config.ResetServerRuntime()
...
func (s *EngineTestSuite) TestFlowStartedEvent_SubjectTypeOmittedWhenCategoryUnknown() {
	mockObs, captured := setupCapturingObservability(s.T())
	defer config.ResetServerRuntime()
	...
	evt := (*captured)[0]
	_, ok := evt.Data[event.DataKey.SubjectType]
	s.False(ok, "subject_type should be omitted when the category is unknown")
	assertEventData(s.T(), evt, map[string]interface{}{event.DataKey.Subject: "user-999"})
}
...
func (s *EngineTestSuite) TestFlowStartedEvent_ApplicationReportsApplicationActorType() {
	mockObs, captured := setupCapturingObservability(s.T())
	defer config.ResetServerRuntime()
...
func (s *EngineTestSuite) TestFlowStartedEvent_OmitsUnknownPrincipalAndSubjectFields() {
	mockObs, captured := setupCapturingObservability(s.T())
	defer config.ResetServerRuntime()
	...
	for _, key := range []string{
		...
		_, ok := evt.Data[key]
		s.False(ok, "expected %q to be omitted", key)
	}
	s.Equal("flow-exec-2", evt.Data[event.DataKey.CorrelationID])
}

func (s *EngineTestSuite) TestNodeExecutionStartedEvent_ReportsSubjectAndPrincipal() {
	t := s.T()
...
func (s *EngineTestSuite) TestNodeExecutionCompletedEvent_SubjectComesFromTheNodeResponse() {
	t := s.T()
	mockObs, captured := setupCapturingObservability(t)
	defer config.ResetServerRuntime()
...
func (s *EngineTestSuite) TestFlowCompletedEvent_ReportsPrincipalAndCorrelation() {
	mockObs, captured := setupCapturingObservability(s.T())
	defer config.ResetServerRuntime()
...
func (s *EngineTestSuite) TestFlowFailedEvent_SharesTheFlowTrace() {
	mockObs, captured := setupCapturingObservability(s.T())
	defer config.ResetServerRuntime()
