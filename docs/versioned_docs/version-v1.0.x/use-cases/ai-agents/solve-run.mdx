...
| Delegated token | the customer | the agent | signing the customer in |
...
- Work it does for a person who signs in names both of them, so a service sees the customer and the agent.
