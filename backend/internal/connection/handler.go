...
func (h *handler) createAuthZENPDPConnection(w http.ResponseWriter, r *http.Request) {
	...
	created, svcErr := h.svc.createAuthZENPDP(ctx, h.svc.authZENPDPService.FromRequest(*req))
	if svcErr != nil {
		writeServiceError(ctx, w, svcErr)
		return
	}
...
func (h *handler) updateAuthZENPDPConnection(w http.ResponseWriter, r *http.Request) {
	...
	updated, svcErr := h.svc.updateAuthZENPDP(ctx, id, h.svc.authZENPDPService.FromRequest(*req))
	if svcErr != nil {
		writeServiceError(ctx, w, svcErr)
		return
	}
