...
The console's connection edit page gains an "Authorization Mapping" card under the Attributes tab. Each
row maps one claim; by default its values are matched by name against an existing role, group, or
permission:

![Direct mode](assets/direct-mode.png)

An Advanced Rules toggle switches the row to explicit value-to-target rules instead, each granting a
combination of roles, groups, and permissions:
