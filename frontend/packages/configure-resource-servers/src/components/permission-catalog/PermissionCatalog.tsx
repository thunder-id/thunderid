...
function CatalogResourceNode({
  ...
  const {data: childResourcesData} = useGetResources(resourceServerId, resource.id);
  ...
  return (
    <>
      ...
      <Collapse in={isOpen}>
        <CatalogRow
          name={t('resourceServers:permissionCatalog.thisResource', 'This resource')}
          permission={resource.permission}
          depth={depth + 1}
          checked={isPermissionSelected(selected, resourceServerId, resource.permission)}
          disabled={readOnly}
          onToggle={() => onChange(togglePermission(selected, resourceServerId, resource.permission))}
        />
