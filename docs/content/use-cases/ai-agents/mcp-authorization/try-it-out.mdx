...
Add Inspector's origin to the server-config `cors` section. Create the declarative resource `config/resources/server_configs/cors.yaml` and set `server_config.store: composite` in `deployment.yaml` to load it at boot, or apply it without a restart via `PUT /server-config/cors`:
