...
func (ts *DCRMTestSuite) TestUpdateWithoutClientIDIsRejected() {
	registered := ts.register("DCRM Missing ClientID")
	ts.registeredAppIDs = append(ts.registeredAppIDs, registered.AppID)

	body, err := json.Marshal(DCRRegistrationRequest{
		ClientName:   "DCRM Missing ClientID",
		RedirectURIs: []string{"https://dcrm.example.com/callback"},
	})
	ts.Require().NoError(err)
...
// RFC 7591 marks client_name optional, and a registration may name the client only in localized
// variants. The stored name is then an i18n reference rather than text, so this is the registration
// shape that a later update must not be able to leave pointing at an empty key.
func (ts *DCRMTestSuite) TestRegisterWithOnlyLocalizedNames() {
	body, err := json.Marshal(DCRRegistrationRequest{
		RedirectURIs:  []string{"https://dcrm.example.com/callback"},
		GrantTypes:    []string{"authorization_code"},
		ResponseTypes: []string{"code"},
		LocalizedFields: map[string]string{
			"client_name#fr": "Application Sans Base",
			"client_name#ja": "ベースなしアプリ",
		},
	})
	ts.Require().NoError(err)
	registered := ts.registerWithBody(body)
	ts.registeredAppIDs = append(ts.registeredAppIDs, registered.AppID)

	read, status, _ := ts.manage(http.MethodGet, registered.ClientID, nil)
	ts.Require().Equal(http.StatusOK, status)
	ts.Require().NotNil(read)
	ts.Assert().Equal("Application Sans Base", read.LocalizedClientName["fr"])
	ts.Assert().Equal("ベースなしアプリ", read.LocalizedClientName["ja"])
	ts.T().Logf("localized-only registration: client_name=%q variants=%v",
		read.ClientName, read.LocalizedClientName)
}

// A read must report the variants a client actually has, not the ones a request happened to send,
// because an update replaces the variants of every field it supplies and a caller has to be able to
// see what it is replacing.
func (ts *DCRMTestSuite) TestReadReturnsStoredLocalizedVariants() {
	body, err := json.Marshal(DCRRegistrationRequest{
		ClientName:    "DCRM Localized Read",
		RedirectURIs:  []string{"https://dcrm.example.com/callback"},
		GrantTypes:    []string{"authorization_code"},
		ResponseTypes: []string{"code"},
		LocalizedFields: map[string]string{
			"client_name#fr": "Lecture Localisee",
			"client_name#ja": "ローカライズされた読み取り",
		},
	})
	ts.Require().NoError(err)
	registered := ts.registerWithBody(body)
	...
	// Resolved from the i18n store rather than echoed from a request.
	ts.Assert().Equal("Lecture Localisee", read.LocalizedClientName["fr"])
	ts.Assert().Equal("ローカライズされた読み取り", read.LocalizedClientName["ja"])
	// The base field carries the resolved system-language value, not the stored i18n reference.
	ts.Assert().Equal("DCRM Localized Read", read.ClientName)
	ts.Assert().NotContains(read.ClientName, "{{t(")
}

// RFC 7592 section 2.2 permits a client_secret in an update but requires it to match the issued
// secret. A wrong value is rejected and the registration is left unchanged.
func (ts *DCRMTestSuite) TestUpdateWithMismatchedClientSecretIsRejected() {
	registered := ts.register("DCRM Wrong Secret")
	ts.registeredAppIDs = append(ts.registeredAppIDs, registered.AppID)

	body, err := json.Marshal(DCRRegistrationRequest{
		ClientID:     registered.ClientID,
		ClientSecret: "not-the-issued-secret",
		ClientName:   "DCRM Wrong Secret Renamed",
		RedirectURIs: []string{"https://dcrm.example.com/callback"},
	})
	ts.Require().NoError(err)
	...
	// The registration is untouched: the name is still the one it was registered with.
	current, readStatus, _ := ts.manage(http.MethodGet, registered.ClientID, nil)
	ts.Require().Equal(http.StatusOK, readStatus)
	ts.Require().NotNil(current)
	ts.Assert().Equal("DCRM Wrong Secret", current.ClientName)
}

// A client_secret matching the issued secret is accepted and the update is applied.
func (ts *DCRMTestSuite) TestUpdateWithMatchingClientSecretIsAccepted() {
	registered := ts.register("DCRM Correct Secret")
	ts.registeredAppIDs = append(ts.registeredAppIDs, registered.AppID)
	ts.Require().NotEmpty(registered.ClientSecret, "registration must return a secret to echo back")

	body, err := json.Marshal(DCRRegistrationRequest{
		ClientID:     registered.ClientID,
		ClientSecret: registered.ClientSecret,
		ClientName:   "DCRM Correct Secret Renamed",
		RedirectURIs: []string{"https://dcrm.example.com/callback"},
	})
	ts.Require().NoError(err)
	...
	ts.Assert().Equal("DCRM Correct Secret Renamed", updated.ClientName)
}
...
func (ts *DCRMTestSuite) register(clientName string) *DCRRegistrationResponse {
	...
	ts.Require().NoError(err)
	return ts.registerWithBody(requestJSON)
}

// registerWithBody registers a client from a caller-supplied body, so a test can register with
// localized variants that the fixed helper above does not carry.
func (ts *DCRMTestSuite) registerWithBody(requestJSON []byte) *DCRRegistrationResponse {
	...
	if resp.StatusCode != http.StatusCreated {
		...
		ts.T().Fatalf("Expected status 201 registering client, got %d. Response: %s",
