...
func (s *scimGroupsService) ListGroups(ctx context.Context, startIndex, count int,
	...
	for _, g := range listResp.Groups {
		...
		if svcErr != nil {
			s.logger.Error(ctx, "SCIM ListGroups: failed to fetch group members",
...
func (s *scimGroupsService) GetGroup(ctx context.Context, groupID, baseURL string,
	...
	if svcErr != nil {
		s.logger.Debug(ctx, "SCIM GetGroup: group service error",
	...
	if svcErr != nil {
		s.logger.Error(ctx, "SCIM GetGroup: failed to fetch group members",
			log.String("groupID", groupID), log.Any("error", svcErr))
		return nil, mapGroupServiceErrorToSCIM(svcErr)
	}
	g.Members = members
...
func (s *scimGroupsService) CreateGroup(ctx context.Context, displayName string,
	...
	if svcErr != nil {
		s.logger.Debug(ctx, "SCIM CreateGroup: invalid member type", log.Any("error", svcErr))
		return nil, svcErr
	}
	...
	if svcErr != nil {
		s.logger.Error(ctx, "SCIM CreateGroup: group service error", log.Any("error", svcErr))
		return nil, mapGroupServiceErrorToSCIM(svcErr)
	}
	return s.GetGroup(ctx, created.ID, baseURL)
}

// ReplaceGroup replaces a group's displayName and members.
func (s *scimGroupsService) ReplaceGroup(ctx context.Context, groupID, displayName string,
	members []SCIMGroupMember, baseURL string) (*SCIMGroup, *tidcommon.ServiceError) {
	thunderIDMembers, svcErr := scimMembersToThunderID(members)
	if svcErr != nil {
		s.logger.Debug(ctx, "SCIM ReplaceGroup: invalid member type", log.Any("error", svcErr))
		return nil, svcErr
	}

	g, svcErr := s.groupService.GetGroup(ctx, groupID, false)
	if svcErr != nil {
		s.logger.Debug(ctx, "SCIM ReplaceGroup: group service error",
			log.String("groupID", groupID), log.Any("error", svcErr))
		return nil, mapGroupServiceErrorToSCIM(svcErr)
	}
	if g.IsReadOnly {
		s.logger.Debug(ctx, "SCIM ReplaceGroup: group is read-only", log.String("groupID", groupID))
		return nil, &scim.ErrorMutabilityViolation
	}
	...
	if svcErr != nil {
		s.logger.Error(ctx, "SCIM ReplaceGroup: failed to update group",
			log.String("groupID", groupID), log.Any("error", svcErr))
		return nil, mapGroupServiceErrorToSCIM(svcErr)
	}

	if svcErr := s.replaceGroupMembers(ctx, groupID, thunderIDMembers, "ReplaceGroup"); svcErr != nil {
		return nil, svcErr
	}
	return s.GetGroup(ctx, groupID, baseURL)
}

// PatchGroup applies a sequence of validated PATCH actions to a group.
func (s *scimGroupsService) PatchGroup(ctx context.Context, groupID string, actions []SCIMGroupPatchAction,
	baseURL string) (*SCIMGroup, *tidcommon.ServiceError) {
	g, svcErr := s.groupService.GetGroup(ctx, groupID, false)
	if svcErr != nil {
		s.logger.Debug(ctx, "SCIM PatchGroup: group service error",
			log.String("groupID", groupID), log.Any("error", svcErr))
		return nil, mapGroupServiceErrorToSCIM(svcErr)
	}
	if g.IsReadOnly {
		s.logger.Debug(ctx, "SCIM PatchGroup: group is read-only", log.String("groupID", groupID))
		return nil, &scim.ErrorMutabilityViolation
	}
...
func (s *scimGroupsService) DeleteGroup(ctx context.Context, groupID string,
) *tidcommon.ServiceError {
	g, svcErr := s.groupService.GetGroup(ctx, groupID, false)
	if svcErr != nil {
		s.logger.Debug(ctx, "SCIM DeleteGroup: group service error",
			log.String("groupID", groupID), log.Any("error", svcErr))
		return mapGroupServiceErrorToSCIM(svcErr)
	}
	if g.IsReadOnly {
		s.logger.Debug(ctx, "SCIM DeleteGroup: group is read-only", log.String("groupID", groupID))
		return &scim.ErrorMutabilityViolation
	}
	svcErr = s.groupService.DeleteGroup(ctx, groupID)
	if svcErr != nil {
		s.logger.Error(ctx, "SCIM DeleteGroup: failed to delete group",
			log.String("groupID", groupID), log.Any("error", svcErr))
	}
	return mapGroupServiceErrorToSCIM(svcErr)
}
...
func (s *scimGroupsService) applyDisplayNamePatch(ctx context.Context, groupID, ouID, description string,
	...
	if _, svcErr := s.groupService.UpdateGroup(ctx, groupID, req); svcErr != nil {
		s.logger.Error(ctx, "SCIM PatchGroup: failed to update displayName",
			log.String("groupID", groupID), log.Any("error", svcErr))
		return mapGroupServiceErrorToSCIM(svcErr)
	}
	return nil
...
func (s *scimGroupsService) applyMembersPatch(ctx context.Context, groupID string,
	...
	case scimPatchOpAdd:
		...
		if svcErr != nil {
			s.logger.Debug(ctx, "SCIM PatchGroup: invalid member type on add",
				log.String("groupID", groupID), log.Any("error", svcErr))
			return svcErr
		}
		...
		if _, svcErr := s.groupService.AddGroupMembers(ctx, groupID, thunderIDMembers); svcErr != nil {
			s.logger.Error(ctx, "SCIM PatchGroup: failed to add members",
				log.String("groupID", groupID), log.Any("error", svcErr))
			return mapGroupServiceErrorToSCIM(svcErr)
		}
		return nil
	...
	case scimPatchOpReplace:
		thunderIDMembers, svcErr := scimMembersToThunderID(action.Members)
		if svcErr != nil {
			s.logger.Debug(ctx, "SCIM PatchGroup: invalid member type on replace",
				log.String("groupID", groupID), log.Any("error", svcErr))
			return svcErr
		}
		return s.replaceGroupMembers(ctx, groupID, thunderIDMembers, "PatchGroup")
	}
	return nil
}

// replaceGroupMembers makes the group's membership equal to desired by removing only the members
// absent from desired and then adding only those not yet present. Members that stay are never
// touched. KNOWN LIMITATION: the group service has no atomic set-members operation, so the remove
// and add run as separate transactions. Removing first means a failure between them leaves the
// group with fewer members than requested, never more.
func (s *scimGroupsService) replaceGroupMembers(ctx context.Context, groupID string, desired []group.Member,
	caller string) *tidcommon.ServiceError {
	existing, svcErr := s.fetchAllGroupMembers(ctx, groupID, false)
	if svcErr != nil {
		s.logger.Error(ctx, "SCIM "+caller+": failed to fetch existing members",
			log.String("groupID", groupID), log.Any("error", svcErr))
		return mapGroupServiceErrorToSCIM(svcErr)
	}
	...
	wanted := make(map[memberKey]struct{}, len(desired))
	for _, m := range desired {
		wanted[memberKey{m.Type, m.ID}] = struct{}{}
	}
	present := make(map[memberKey]struct{}, len(existing))
	var toRemove []group.Member
	for _, m := range existing {
		key := memberKey{m.Type, m.ID}
		present[key] = struct{}{}
		if _, keep := wanted[key]; !keep {
			toRemove = append(toRemove, m)
		}
	}
	var toAdd []group.Member
	for _, m := range desired {
		key := memberKey{m.Type, m.ID}
	...
	if len(toRemove) > 0 {
		if _, svcErr := s.groupService.RemoveGroupMembers(ctx, groupID, toRemove); svcErr != nil {
			s.logger.Error(ctx, "SCIM "+caller+": failed to remove members",
				log.String("groupID", groupID), log.Any("error", svcErr))
			return mapGroupServiceErrorToSCIM(svcErr)
		}
	}
	if len(toAdd) > 0 {
		if _, svcErr := s.groupService.AddGroupMembers(ctx, groupID, toAdd); svcErr != nil {
			s.logger.Error(ctx, "SCIM "+caller+": failed to add members",
				log.String("groupID", groupID), log.Any("error", svcErr))
			return mapGroupServiceErrorToSCIM(svcErr)
		}
	}
...
// applyMembersRemove removes members matching filterValue or all members if filterValue is empty.
func (s *scimGroupsService) applyMembersRemove(ctx context.Context, groupID, filterValue string,
	...
	if svcErr != nil {
		s.logger.Error(ctx, "SCIM PatchGroup: failed to fetch existing members for remove",
			log.String("groupID", groupID), log.Any("error", svcErr))
		return mapGroupServiceErrorToSCIM(svcErr)
	}
	...
	if _, svcErr := s.groupService.RemoveGroupMembers(ctx, groupID, toRemove); svcErr != nil {
		s.logger.Error(ctx, "SCIM PatchGroup: failed to remove members",
			log.String("groupID", groupID), log.Any("error", svcErr))
		return mapGroupServiceErrorToSCIM(svcErr)
	}
	return nil
...
// fetchAllGroupMembers retrieves all members of a group across pagination boundaries.
func (s *scimGroupsService) fetchAllGroupMembers(ctx context.Context, groupID string, includeDisplay bool,
	...
	for {
		...
		if svcErr != nil {
			s.logger.Error(ctx, "SCIM: failed to list group members",
...
func scimMembersToThunderID(members []SCIMGroupMember) ([]group.Member, *tidcommon.ServiceError) {
	...
	for _, m := range members {
		...
		case strings.EqualFold(m.Type, "App"):
			mt = group.MemberTypeApp
		case strings.EqualFold(m.Type, "Agent"):
			mt = group.MemberTypeAgent
		default:
			return nil, &scim.ErrorUnsupportedMemberType
		}
		result = append(result, group.Member{ID: m.Value, Type: mt})
	}
	return result, nil
}
