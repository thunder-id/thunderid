---
title: Overview
docType: community
description: Backend contribution guidelines for the ThunderID Go application, covering structure and testing.
sidebar_position: 1
hide_table_of_contents: false
---

# Backend Contribution Guidelines

The backend is a Go application under `backend/`.

## Project Structure

This is the flat layout to follow for a **new** domain package under `internal/`. Existing packages vary: some register
their routes in `init.go` with no separate `handler.go`, a few use a `model/` subpackage instead of a single `model.go`,
and `constants.go` is only present where a package actually needs shared constants. Match the package you are working in
rather than adding files to make it fit this table.

| File | Purpose |
|------|---------|
| `service.go` | Service interface + business logic |
| `handler.go` | HTTP handlers |
| `store.go` | Data access layer |
| `model.go` | Domain models and DTOs |
| `constants.go` | Package constants |
| `error_constants.go` | Error messages and codes |
| `store_constants.go` | DB queries and table names |
| `init.go` | Package initialization and route registration |

## Key Rules

- **File naming**: `snake_case` (e.g., `error_constants.go`)
- **Exports**: Only export service interface and models used in service. Keep implementations, constants, queries unexported.
- **Logging**: Use the `log` package from `internal/system`. Pass the request `context.Context` as the first argument to the logging methods (e.g. `Info(ctx, …)`/`Error(ctx, …)`) so entries carry the correlation (trace) ID. For non-request code (startup, shutdown, detached goroutines), pass `context.Background()`, or `context.WithoutCancel(ctx)` to keep the trace ID across a goroutine, with a short comment explaining why no request context is available. Avoid PII; use `MaskString` for sensitive data.

## Mock Generation

After changing any interface, regenerate mocks:

```bash
make mockery
```

:::warning
CI has a `verify-mocks` job that will fail your build if mocks are out of sync with the interfaces.
:::

## Testing

| Purpose | Command |
|---------|---------|
| Unit tests | `make test_unit` |
| Integration tests | `make test_integration` |
| Specific test | `make test_integration RUN="TestName"` |
| Specific package | `make test_integration PACKAGE="pkg/path"` |
| All backend tests | `make test` |
