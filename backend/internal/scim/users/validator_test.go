...
func (suite *ValidatorTestSuite) TestValidateSCIMUserRequest() {
	t := suite.T()
	...
	tests := []struct {
		...
		},
		{
			...
			body: []byte(
				`{"schemas":["` + scim.SCIMCoreUserSchemaURN + `","` + validURN + `"],"userName":"alice"}`),
			wantErrCode:  "",
		...
		{
			name: "EnterpriseSchema_Valid",
			body: []byte(`{
				"schemas":["` + scim.SCIMCoreUserSchemaURN + `","` + scim.SCIMEnterpriseUserSchemaURN + `",` +
				`"` + validURN + `"],
				"` + validURN + `":{"department":"engineering"},
				"` + scim.SCIMEnterpriseUserSchemaURN + `":{"department":"engineering","employeeNumber":"123"},
				"userName":"alice"
			}`),
			wantErrCode:  "",
			wantUserType: "employee",
		...
		{
			name: "EnterpriseSchema_UndeclaredObject_ReturnsError",
			body: []byte(`{
				"schemas":["` + scim.SCIMCoreUserSchemaURN + `"],
				"` + scim.SCIMEnterpriseUserSchemaURN + `":{"department":"engineering"},
				"userName":"alice"
			}`),
			wantErrCode: scim.ErrorUndeclaredEnterpriseSchemaObject.Code,
		},
		{
			name: "EnterpriseSchema_InvalidObjectJSON",
			body: []byte(`{
				"schemas":["` + scim.SCIMCoreUserSchemaURN + `","` + scim.SCIMEnterpriseUserSchemaURN + `"],
				"` + scim.SCIMEnterpriseUserSchemaURN + `":"not-an-object",
				"userName":"alice"
			}`),
			wantErrCode: scim.ErrorInvalidRequestBody.Code,
		},
		{
			name: "CoreAndEnterprise_NoCustomURN",
			body: []byte(`{
				"schemas":["` + scim.SCIMCoreUserSchemaURN + `","` + scim.SCIMEnterpriseUserSchemaURN + `"],
				"` + scim.SCIMEnterpriseUserSchemaURN + `":{"department":"engineering"},
				"userName":"alice"
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			payload, svcErr := parseAndValidateSCIMUserRequest(tc.body, testURNPrefix)
			if tc.wantErrCode != "" {
...
func (suite *ValidatorTestSuite) TestMissingRequiredAttrs_InvalidSchemaJSON() {
	t := suite.T()
...
func (suite *ValidatorTestSuite) TestMissingRequiredAttrs_AllPresent() {
	t := suite.T()
...
func (suite *ValidatorTestSuite) TestMissingRequiredAttrs_ReportsMissingSorted() {
	t := suite.T()
...
func (suite *ValidatorTestSuite) TestMissingRequiredAttrs_SkipCredentialTrue_OmitsCredential() {
	t := suite.T()
...
func (suite *ValidatorTestSuite) TestMissingRequiredAttrs_SkipCredentialFalse_IncludesCredential() {
	t := suite.T()
	schema := json.RawMessage(`{"password":{"required":true,"credential":true}}`)
	missing, err := missingRequiredAttrs(map[string]json.RawMessage{}, schema, false)
	require.NoError(t, err)
...
func (suite *ValidatorTestSuite) TestUndeclaredAttrs_ReportsUndeclaredSorted() {
	t := suite.T()
	schema := json.RawMessage(`{
		"given_name":{"required":true}
	}`)
...
func (suite *ValidatorTestSuite) TestMissingRequiredAttrs_CaseInsensitive() {
	t := suite.T()
	schema := json.RawMessage(`{"given_name":{"required":true}}`)
	attrs := map[string]json.RawMessage{"Given_Name": json.RawMessage(`"Alice"`)}
	missing, err := missingRequiredAttrs(attrs, schema, false)
...
func (suite *ValidatorTestSuite) TestUndeclaredAttrs_CaseInsensitive() {
	t := suite.T()
	schema := json.RawMessage(`{"given_name":{"required":true}}`)
	extAttrs := map[string]json.RawMessage{
		"Given_Name": json.RawMessage(`"Alice"`),
...
// TestParseAndValidateSCIMUserRequest_IgnoresReadOnlyCommonAttrs tests that a non-core user's GET body
// echoed back (id, meta, externalId beside the extension URN only) yields no core attributes.
func (suite *ValidatorTestSuite) TestParseAndValidateSCIMUserRequest_IgnoresReadOnlyCommonAttrs() {
	t := suite.T()
	ext := "urn:thunderid:params:scim:schemas:employee:2.0:User"
	body := []byte(`{
		"schemas": ["` + ext + `"],
		"id": "user-1",
		"externalId": "ext-1",
