...
func (r *entityTypeResolution) resolve(ctx *providers.NodeContext, category entitytype.TypeCategory,
	...
	typeKey := runtimeEntityTypeKey(category)

	if selected, ok := ctx.UserInputs[typeKey]; ok && selected != "" {
		return r.resolveSubmitted(ctx, category, selected, allowedTypes, execResp, logger)
	}
	...
	if len(candidates) == 1 {
		return r.accept(ctx, category, candidates[0].Name, candidates[0].OUID, execResp, logger), nil
	}
...
func (r *entityTypeResolution) resolveSubmitted(ctx *providers.NodeContext,
	...
	if len(allowedTypes) > 0 && !slices.Contains(allowedTypes, selected) {
		logger.Debug(ctx.Context, "Entity type not in allowed list", log.String(typeKey, selected),
	...
	if selectedOUID, exists := ctx.RuntimeData[ouIDKey]; exists && selectedOUID != "" {
		...
		if svcErr != nil {
			logger.Error(ctx.Context, "Failed to validate the entity type against the selected OU",
				log.String(typeKey, selected), log.String(ouIDKey, selectedOUID),
	...
	logger.Debug(ctx.Context, "Entity type resolved from input", log.String(typeKey, selected),
		log.String(ouIDKey, entityType.OUID))

	return r.accept(ctx, category, selected, ouID, execResp, logger), nil
}
...
func (r *entityTypeResolution) accept(ctx *providers.NodeContext, category entitytype.TypeCategory,
	name, ouID string, execResp *providers.ExecutorResponse, logger *log.Logger,
) *providers.ExecutorResponse {
	typeKey := runtimeEntityTypeKey(category)

	logger.Debug(ctx.Context, "Entity type resolved", log.String(typeKey, name), log.String(ouIDKey, ouID))
	execResp.RuntimeData[typeKey] = name
