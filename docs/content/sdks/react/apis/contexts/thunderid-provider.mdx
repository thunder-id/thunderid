...
createRoot(document.getElementById('root')).render(
  <StrictMode>
    <ThunderIDProvider
      clientId="<your-app-client-id>"
      baseUrl="https://localhost:8090"
      scopes="openid profile internal_organization_create internal_organization_view internal_organization_update internal_organization_delete"
...
| `scopes` | `string \| string[]` | ❌ | OpenID Connect Scopes to request during the sign-in. Defaults to `'openid profile'` if not provided. |
