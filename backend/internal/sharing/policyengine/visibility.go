...
// is, so a missing or excluded link cuts off everything below it. Returning all covering policies
// rather than the nearest is what lets rule resolution intersect them.
...
// dedupe drops repeated (policy, target) pairs, which a chain with several covered ancestors can
// otherwise produce.
