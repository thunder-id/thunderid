...
/*
Token exchange AuthorizationRuleMapping integration tests.

These exercise the connection's authorizationMapping.rules section on the RFC 8693 token exchange path: a
...
// TokenExchangeAuthorizationRuleMappingTestSuite runs its own mock external OIDC issuer and its own
// resource servers/roles/group, independent of TokenExchangeTestSuite, so its connection's
// authorizationMapping.rules can be rewritten per scenario without disturbing other token exchange tests.
type TokenExchangeAuthorizationRuleMappingTestSuite struct {
	suite.Suite
	client *http.Client
...
func TestTokenExchangeAuthorizationRuleMappingTestSuite(t *testing.T) {
	suite.Run(t, new(TokenExchangeAuthorizationRuleMappingTestSuite))
}

func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) SetupSuite() {
...
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) TearDownSuite() {
	if ts.exchangeAppID != "" {
...
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) applyAuthorizationRuleMapping(
...
// explicit rules, for scenarios that need an operator other than equals (applyAuthorizationRuleMapping
// only ever builds equals rules).
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) applyAuthorizationRuleMappingRules(
	claim, valueType string, rules []testutils.AuthorizationRule,
) {
	ts.applyAuthorizationRuleMappingRulesAndDelimiter(claim, valueType, "", rules)
}

// applyAuthorizationRuleMappingRulesAndDelimiter is applyAuthorizationRuleMappingRules with an explicit
// delimiter, for a multi-valued string claim (a delimiter is only meaningful for the string value
// type).
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) applyAuthorizationRuleMappingRulesAndDelimiter(
...
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) createExchangeApplication() string {
...
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) nextSubject() string {
	ts.subCounter++
...
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) mintExternalIDToken(sub string) string {
	ts.T().Helper()
	ts.activeSub = sub
...
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) exchangeToken(idToken, scope, resource string) (
...
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) TestTokenExchange_MappedRoleGrantsScopeBeyondNativeScope() {
	sub := ts.nextSubject()
	ts.mockIDP.AddUser(&testutils.OIDCUserInfo{
		Sub:    sub,
		Custom: map[string]interface{}{"groups": []interface{}{"te-admins"}},
	})
	ts.applyAuthorizationRuleMapping("groups", "", map[string][]testutils.AuthorizationTarget{
		"te-admins": {{Type: testutils.AuthorizationTargetRole, ID: ts.mappedRoleID}},
	})

	idToken := ts.mintExternalIDToken(sub)
	resp, status := ts.exchangeToken(idToken, "read write", teAuthzRSIdentifier)
	...
	ts.Contains(scopes, "read")
	ts.NotContains(scopes, "write", "the mapped role only grants read, not every permission on the resource server")
}

// TestTokenExchange_EmptyScopeWithMappingConfiguredGrantsNothing covers a request that omits scope
// entirely while the connection's authorization mapping is configured. There is nothing requested to
// evaluate against the mapping, and the subject token's own scope claim is not an authority once
// mapping is configured, so no scope is granted, and the exchange still succeeds.
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) TestTokenExchange_EmptyScopeWithMappingConfiguredGrantsNothing() {
	sub := ts.nextSubject()
	ts.mockIDP.AddUser(&testutils.OIDCUserInfo{
		Sub:    sub,
		Custom: map[string]interface{}{"groups": []interface{}{"te-admins"}},
	...
	idToken := ts.mintExternalIDToken(sub)
	resp, status := ts.exchangeToken(idToken, "", teAuthzRSIdentifier)
	ts.Require().Equal(http.StatusOK, status, "error=%s description=%s", resp.Error, resp.ErrorDescription)
	ts.Require().NotEmpty(resp.AccessToken)
	ts.Empty(resp.Scope, "no scope was requested, so the mapping (the sole authority once configured) has nothing to grant")
}
...
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) TestTokenExchange_MappedGroupGrantsScope() {
	sub := ts.nextSubject()
	ts.mockIDP.AddUser(&testutils.OIDCUserInfo{
		Sub:    sub,
		Custom: map[string]interface{}{"department": "platform"},
	})
	ts.applyAuthorizationRuleMapping("department", "", map[string][]testutils.AuthorizationTarget{
	...
	ts.Require().Equal(http.StatusOK, status, "error=%s description=%s", resp.Error, resp.ErrorDescription)
	scopes := strings.Fields(resp.Scope)
	ts.Contains(scopes, "write")
	ts.NotContains(scopes, "read", "the mapped group's role only grants write, not every permission on the resource server")
}
...
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) TestTokenExchange_MappedPermissionTargetGrantsDirectly() {
	sub := ts.nextSubject()
	ts.mockIDP.AddUser(&testutils.OIDCUserInfo{
		Sub:    sub,
		Custom: map[string]interface{}{"scope_claim": "delegate"},
	})
	ts.applyAuthorizationRuleMapping("scope_claim", "", map[string][]testutils.AuthorizationTarget{
		"delegate": {{
	...
	scopes := strings.Fields(resp.Scope)
	ts.Contains(scopes, "read")
	ts.NotContains(scopes, "write", "a mapped permission target grants only the permission it names")
}
...
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) TestTokenExchange_UnmappedValueGrantsNoScope() {
	sub := ts.nextSubject()
	ts.mockIDP.AddUser(&testutils.OIDCUserInfo{
		Sub:    sub,
...
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) TestTokenExchange_ListValuedClaimGrantsRole() {
	sub := ts.nextSubject()
	ts.mockIDP.AddUser(&testutils.OIDCUserInfo{
		Sub:    sub,
	...
	scopes := strings.Fields(resp.Scope)
	ts.Contains(scopes, "read")
	ts.NotContains(scopes, "write", "the mapped role only grants read")
}
...
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) TestTokenExchange_NotEqualsOperatorMatchesElementInListClaim() {
	sub := ts.nextSubject()
	ts.mockIDP.AddUser(&testutils.OIDCUserInfo{
		Sub:    sub,
		Custom: map[string]interface{}{"groups": []interface{}{"guest", "engineering"}},
	})
	ts.applyAuthorizationRuleMappingRules("groups", "", []testutils.AuthorizationRule{
		{
...
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) TestTokenExchange_NotIncludesWithholdsWhenExcludedValuePresentInListClaim() {
	ts.applyAuthorizationRuleMappingRules("groups", testutils.AuthorizationValueTypeArray, []testutils.AuthorizationRule{
		{
...
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) TestTokenExchange_IncludesOperatorTestsSetMembershipInListClaim() {
	sub := ts.nextSubject()
	ts.mockIDP.AddUser(&testutils.OIDCUserInfo{
		Sub:    sub,
		Custom: map[string]interface{}{"groups": []interface{}{"engineering", "te-admins"}},
...
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) TestTokenExchange_SpaceDelimitedClaimGrantsRole() {
	sub := ts.nextSubject()
	ts.mockIDP.AddUser(&testutils.OIDCUserInfo{
		Sub:    sub,
		Custom: map[string]interface{}{"scope_claim": "orders.read orders.write"},
	})
	ts.applyAuthorizationRuleMappingRulesAndDelimiter("scope_claim", testutils.AuthorizationValueTypeString, " ",
...
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) TestTokenExchange_PermissionTargetScopedToItsOwnResourceServer() {
...
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) TestTokenExchange_NoLocalRecordRequired() {
	sub := ts.nextSubject()
	ts.mockIDP.AddUser(&testutils.OIDCUserInfo{
		Sub:    sub,
		Custom: map[string]interface{}{"groups": []interface{}{"te-admins"}},
...
// group's own ancestor group grants, matching the federated-login counterpart of this scenario.
func (ts *TokenExchangeAuthorizationRuleMappingTestSuite) TestTokenExchange_MappedChildGroupGrantsParentGroupScope() {
	childGroupID, err := testutils.CreateGroup(testutils.Group{
		Name: "TEAncChild " + ts.nextSubject(),
		OUID: ts.ouID,
	})
	ts.Require().NoError(err, "failed to create the child group")
	defer func() {
		if err := testutils.DeleteGroup(childGroupID); err != nil {
			ts.T().Logf("failed to delete the child group: %v", err)
		}
	}()

	parentGroupID, err := testutils.CreateGroup(testutils.Group{
		Name:    "TEAncParent " + ts.nextSubject(),
		OUID:    ts.ouID,
		Members: []testutils.Member{{Id: childGroupID, Type: "group"}},
	})
	ts.Require().NoError(err, "failed to create the parent group with the child nested inside it")
	defer func() {
		if err := testutils.DeleteGroup(parentGroupID); err != nil {
			ts.T().Logf("failed to delete the parent group: %v", err)
		}
	}()

	parentRoleID, err := testutils.CreateRole(testutils.Role{
		Name: "TEAncPRole " + ts.nextSubject(),
		OUID: ts.ouID,
		Permissions: []testutils.ResourcePermissions{
	...
	ts.Require().NoError(err, "failed to create the role assigned to the parent group")
	defer func() {
		if err := testutils.DeleteRole(parentRoleID); err != nil {
			ts.T().Logf("failed to delete the parent group's role: %v", err)
		}
	}()
	...
	ts.applyAuthorizationRuleMapping("groups", "", map[string][]testutils.AuthorizationTarget{
		"engineering": {{Type: testutils.AuthorizationTargetGroup, ID: childGroupID}},
	})
