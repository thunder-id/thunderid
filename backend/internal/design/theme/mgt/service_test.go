...
func (s *stubUsageRegistry) RegisterProvider(usage.Provider) {}

func (s *stubUsageRegistry) GetUsages(
	_ context.Context, _, _ string) (*usage.UsagesResponse, error) {
	return s.resp, s.err
}
...
func (suite *ThemeServiceTestSuite) TestGetThemeUsages_WithUsages() {
	suite.mockStore.On("IsThemeExist", "theme-123").Return(true, nil)
	total := 2
	suite.service.SetUsageRegistry(&stubUsageRegistry{
		resp: &usage.UsagesResponse{
			TotalResults: &total,
			Count:        2,
			Summary:      map[string]int{usage.ResourceTypeApplication: 2},
			Usages: []usage.ResourceUsage{
				...
				{ResourceType: usage.ResourceTypeApplication, ID: "app-2",
					DisplayName: "App Two", BehaviorOnDelete: usage.BehaviorFallback},
			},
		},
	})
	...
	assert.Equal(suite.T(), 2, *result.TotalResults)
	assert.Equal(suite.T(), 2, result.Summary[usage.ResourceTypeApplication])
	assert.Len(suite.T(), result.Usages, 2)
	assert.Equal(suite.T(), usage.ResourceTypeApplication, result.Usages[0].ResourceType)
	assert.Equal(suite.T(), usage.BehaviorFallback, result.Usages[0].BehaviorOnDelete)
...
func (suite *ThemeServiceTestSuite) TestGetThemeUsages_NoUsages() {
	...
	total := 0
	suite.service.SetUsageRegistry(&stubUsageRegistry{
		resp: &usage.UsagesResponse{TotalResults: &total, Usages: []usage.ResourceUsage{}},
	})
...
func (suite *ThemeServiceTestSuite) TestGetThemeUsages_RegistryError() {
	suite.mockStore.On("IsThemeExist", "theme-123").Return(true, nil)
	suite.service.SetUsageRegistry(&stubUsageRegistry{err: errors.New("registry error")})
