...
2. Add the local development origin of the <ProductName /> Gate application (https://localhost:5190) to the CORS allowed origins. Create the declarative resource file (source checkout: `backend/cmd/server/config/resources/server_configs/cors.yaml`; packaged distribution: `<THUNDERID_HOME>/config/resources/server_configs/cors.yaml`) and set `server_config.store: composite` in `deployment.yaml` so it loads at boot (the default, `mutable`, does not load declarative resources).
...
5. Add the local development origin of the console application (https://localhost:5191) to the CORS allowed origins. Create the declarative resource file (source checkout: `backend/cmd/server/config/resources/server_configs/cors.yaml`; packaged distribution: `<THUNDERID_HOME>/config/resources/server_configs/cors.yaml`) and set `server_config.store: composite` in `deployment.yaml` so it loads at boot (the default, `mutable`, does not load declarative resources).
