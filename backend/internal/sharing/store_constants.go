...
func buildRelevantPoliciesQuery(
	...
	for i, id := range chainOUIDs {
		postgresPlaceholders[i] = fmt.Sprintf("$%d", i+3)
		sqlitePlaceholders[i] = "?"
		args = append(args, id)
	}

	build := func(placeholders []string, deployment, resource string) string {
		...
		return fmt.Sprintf(
			...
			JOIN "RESOURCE_SHARING_POLICY_TARGET" t ON t.POLICY_ID = p.ID
			WHERE p.DEPLOYMENT_ID = %s AND p.RESOURCE_TYPE = %s
			AND (%s)`,
			deployment, resource, reach)
	}

	postgresQuery := build(postgresPlaceholders, "$1", "$2")
	sqliteQuery := build(sqlitePlaceholders, "?", "?")
