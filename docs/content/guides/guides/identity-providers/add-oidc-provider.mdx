...
The mapping applies both when a user signs in through this IdP in an authentication flow and when a token from this IdP is exchanged (see [Trusted Issuer](../../trusted-issuer#claim-details)). The resulting local attributes flow into the issued access and ID tokens, subject to the application's `userAttributes` configuration.
