...
4. **[Build the Flows](../build-flows)** builds the registration, account-recovery, and sign-out journeys.
5. **[Register the Application](../build-application)** registers the OAuth2 client that runs them.
6. **[Customize the Recovery Email](../build-recovery-email)** tailors the recovery email's content, design, and languages.
7. **[Onboard Internal Staff](../build-onboarding)** adds staff roles and an invitation flow.
8. **[Keep Them Signed In](../build-sessions)** adds the session checkpoint, so a return trip skips the sign-in screen.
