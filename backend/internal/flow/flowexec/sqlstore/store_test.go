...
// Test-local aliases to the flowexec types/functions exercised by this white-box
// store test, so the test body stays identical after relocating to the sqlstore
// package. These shims are test-only and add no production surface.
type (
	EngineContext = flowexec.EngineContext
	FlowContextDB = flowexec.FlowContextDB
)

var FromEngineContext = flowexec.FromEngineContext

// flowContextContent mirrors the unexported flowexec serialization shape so the
// test can assert on individual context fields after JSON (de)serialization.
