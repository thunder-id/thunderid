...
The setup job generates a random admin password and stores it in the `{{productSlug}}-admin-credentials` Secret. Retrieve it to sign in to the Console:

```bash
kubectl get secret {{productSlug}}-admin-credentials -o jsonpath="{.data.admin-password}" | base64 -d; echo
```

Open `https://localhost:8090/console` and sign in as `admin` with that password.
