...
func (s *ExternalAuthZENPDPIntegrationSuite) SetupSuite() {
	...
	connectionBody := map[string]interface{}{
		...
		"timeoutMs":               750,
		"retryCount":              2,
		"subjectProperties":       []string{"username"},
		"subjectPropertyMappings": map[string]string{"username": "preferred_username"},
...
func (s *ExternalAuthZENPDPIntegrationSuite) TestExternalPDPConnectionSettingsAndUsagePersist() {
	...
	s.Equal([]string{"username"}, connection.SubjectProperties)
	s.Equal(map[string]string{"username": "preferred_username"}, connection.SubjectPropertyMappings)
	...
	updatedPayload := mustJSON(map[string]interface{}{
		...
		"timeoutMs":               900,
		"retryCount":              3,
		"subjectProperties":       []string{"username", "ouId"},
		"subjectPropertyMappings": map[string]string{"username": "preferred_username"},
		"subjectAttributeMappings": []externalAuthZENPDPSubjectAttributeMapping{{
			UserType: "user",
			Attributes: []externalAuthZENPDPSubjectAttributeRow{{
	...
	s.Equal([]string{"username", "ouId"}, updatedConnection.SubjectProperties)
...
func (s *ExternalAuthZENPDPIntegrationSuite) TestResourceServerCanSwitchToRBAC() {
	update := func(engineType, connectionID string) {
		payload := mustJSON(externalAuthZENPDPResourceServerUpdate{
			Name: "External AuthZEN API", Identifier: externalAuthZENPDPResourceIdentifier, OUID: s.ouID,
			AuthorizationEngine: externalAuthZENPDPAuthorizationEngineConfig{
				Type:       engineType,
				Properties: externalAuthZENPDPAuthorizationEngineProps{PDPConnectionID: connectionID},
			},
		})
		...
		s.Require().NoError(err)
		s.Require().Equal(http.StatusOK, response.StatusCode, string(body))
		var result externalAuthZENPDPResourceServerUpdate
		s.Require().NoError(json.Unmarshal(body, &result))
