...
type storeInterface interface {
	...
	GetByDataPlaneID(ctx context.Context, dataPlaneID string) (*Gateway, error)
...
// GetByDataPlaneID finds the gateway already registered for a data plane, so a second registration
// of the same one is refused rather than stored alongside the first.
func (s *store) GetByDataPlaneID(ctx context.Context, dataPlaneID string) (*Gateway, error) {
	dbClient, err := s.dbProvider.GetConfigDBClient()
	if err != nil {
		return nil, fmt.Errorf("failed to get database client: %w", err)
	}
	rows, err := dbClient.QueryContext(ctx, queryGetGatewayByDataPlaneID, dataPlaneID, s.deploymentID)
	if err != nil {
...
func (s *store) Create(ctx context.Context, gw *Gateway, limit int) (*Gateway, error) {
	...
	rows, err := dbClient.QueryContext(ctx, queryInsertGateway,
		gw.ID, gw.Name, gw.DataPlaneID, gw.BaseURL, gw.Key, gw.CACertificate, s.deploymentID, limit)
	if err != nil {
...
func (s *store) Update(ctx context.Context, gw *Gateway) error {
	...
	_, err = dbClient.ExecuteContext(ctx, queryUpdateGateway,
		gw.ID, gw.Name, gw.DataPlaneID, gw.BaseURL, gw.Key, gw.CACertificate, s.deploymentID)
	if err != nil {
...
func gatewayFromRow(row map[string]interface{}) (*Gateway, error) {
	gw := &Gateway{
		...
		DataPlaneID:   asString(row["data_plane_id"]),
