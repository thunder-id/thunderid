...
// Package common provides shared SCIM 2.0 types, error constants, and helpers used across the
// discovery, users, and groups SCIM subpackages.
package common
...
// Mutability represents the SCIM attribute "mutability" characteristic (RFC 7643 §7).
...
// (case-insensitive), whether that property (or any matching property, if more than one
// candidate matches) is marked required, and whether any matching property is marked
// credential - both required and credential are OR-aggregated across every matching
// candidate (not just the first or last), since several candidates can roll into one SCIM
// attribute (e.g. given_name/family_name/middle_name all matching "name").
func HasSchemaMatch(
	rawProps map[string]RawPropertyDef, candidates []string,
) (matched, required, credential bool) {
	for _, candidate := range candidates {
		for propName, propDef := range rawProps {
			if strings.EqualFold(propName, candidate) {
				...
				if propDef.Credential {
					credential = true
				}
			}
		}
	}
	return matched, required, credential
}

// EnterpriseField represents an attribute in the SCIM Enterprise User schema extension (RFC 7643 §4.3).
type EnterpriseField string

// Enterprise User schema extension field names (RFC 7643 §4.3).
const (
	EnterpriseFieldEmployeeNumber EnterpriseField = "employeeNumber"
	EnterpriseFieldCostCenter     EnterpriseField = "costCenter"
	EnterpriseFieldOrganization   EnterpriseField = "organization"
	EnterpriseFieldDivision       EnterpriseField = "division"
	EnterpriseFieldDepartment     EnterpriseField = "department"
	EnterpriseFieldManager        EnterpriseField = "manager"
)

// EnterpriseAttrRule maps a ThunderID attribute candidate to a SCIM enterprise attribute.
type EnterpriseAttrRule struct {
	Candidate string          // ThunderID attr name (matched case-insensitively)
	SCIMField EnterpriseField // Target SCIM enterprise attribute name
