...
func (s *service) buildTargets(
	...
	case children:
		...
		seen := make(map[string]struct{}, len(scope.OUIDs))
...
func (s *service) relevantPolicies(
	...
	declared := s.declaredForResource(rt, resourceID)
	if len(declared) == 0 {
		return policies, nil
	}

	// A declared policy that has been edited was written to the database under its own id, and the
	// stored row is what applies. The narrowed fetch above cannot decide that on its own: an edit
	// that moved the policy's targets away from this chain leaves the stored row out of the result
	// while the file's original still names it, and replaying the file version would restore
	// exactly the reach the edit removed. Which ids exist is therefore asked of the database.
...
func (s *service) ListVisibleResourceIDs(
	...
	if s.declarativeStore != nil {
		policies = append(policies, s.declarativeStore.listForType(rt)...)
	}
