...
func (s *entityService) GetResourceDependencies(
	...
	if svcErr != nil {
		if svcErr.Code == entitytype.ErrorUserTypeNotFound.Code {
			// The type is already gone; nothing left to block on.
			return []resourcedependency.ResourceDependency{}, nil
		}
		return nil, fmt.Errorf("failed to get entity type %q: %s", id, svcErr.ErrorDescription.DefaultValue)
	}
