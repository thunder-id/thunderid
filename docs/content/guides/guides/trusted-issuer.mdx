...
To rename external attributes to local attributes during exchange, set the IDP's `attributeConfiguration` field (separate from `properties`). See [Attribute Configuration](#attribute-configuration).
...
Attribute configuration for token exchange follows the same `attributeConfiguration` structure as federated sign-in. See [Add an OIDC Identity Provider](../identity-providers/add-oidc-provider#attribute-configuration) for the full reference.
...
- No scope mapping is applied. External token scopes use the existing intersection logic. External claims can be renamed to local attributes with the IDP's `attributeConfiguration` (see [Attribute Configuration](#attribute-configuration)); the resulting attributes are still filtered by the app's `userAttributes` configuration.
