...
// NewAuthorizationRequestStore creates a new database-backed authorization request store.
func NewAuthorizationRequestStore(deploymentID string) authz.AuthorizationRequestStoreInterface {
...
func (authzRS *authorizationRequestStore) AddRequest(
	ctx context.Context, value authz.AuthRequestContext,
) (string, error) {
...
func (authzRS *authorizationRequestStore) GetRequest(
	ctx context.Context, key string) (bool, authz.AuthRequestContext, error) {
	if key == "" {
		return false, authz.AuthRequestContext{}, nil
	}

	dbClient, err := authzRS.dbProvider.GetRuntimeDBClient()
	if err != nil {
		return false, authz.AuthRequestContext{}, fmt.Errorf("failed to get database client: %w", err)
	}
	...
	if err != nil {
		return false, authz.AuthRequestContext{}, fmt.Errorf("failed to query authorization request: %w", err)
	}
	...
	if err != nil {
		return false, authz.AuthRequestContext{}, fmt.Errorf("failed to build authorization request context: %w", err)
	}
...
func (authzRS *authorizationRequestStore) getJSONDataBytes(authRequestCtx authz.AuthRequestContext) ([]byte, error) {
...
// buildAuthRequestContextFromResultRow builds an authz.AuthRequestContext from a database result row.
func (authzRS *authorizationRequestStore) buildAuthRequestContextFromResultRow(
	row map[string]interface{},
) (authz.AuthRequestContext, error) {
	var dataJSON string
	if val, ok := row[dbColumnRequestData].(string); ok && val != "" {
		...
		return authz.AuthRequestContext{}, fmt.Errorf("%s is missing or of unexpected type", dbColumnRequestData)
	}

	var requestDataMap map[string]interface{}
	if err := json.Unmarshal([]byte(dataJSON), &requestDataMap); err != nil {
		return authz.AuthRequestContext{}, fmt.Errorf("failed to unmarshal %s JSON: %w", dbColumnRequestData, err)
	}
