...
`UserProfile` is a pre-built form widget for viewing and editing the authenticated user's profile. It loads the user's profile from `/scim2/Me` on mount, renders editable fields, and saves changes via `updateUserProfile`.
...
`BaseUserProfile` exposes the profile state and save action to a `builder` function, giving full control over the form layout.
...
`ProfileState` is passed to the `BaseUserProfile` builder on every update.
