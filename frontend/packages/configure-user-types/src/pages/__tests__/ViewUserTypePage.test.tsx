...
import type {ApiUserType, ApiError, Attribute} from '../../types/user-types';
...
// the property row header both show the attribute id. Ids are distinct and not
// substrings of each other.
const {mockAttributes} = vi.hoisted(() => ({
  mockAttributes: [
    {id: 'phone', dataType: 'string', required: false, unique: false, credential: false},
    {id: 'department', dataType: 'string', required: false, unique: false, credential: false},
    {id: 'level', dataType: 'number', required: false, unique: false, credential: false},
  ] as Attribute[],
}));
...
describe('ViewUserTypePage', () => {
  ...
  describe('Schema Tab', () => {
    ...
    it('adds a new property via the attribute library panel', async () => {
      ...
      await waitFor(() => {
        const header = screen.getAllByText('phone').find((el) => el.classList.contains('MuiTypography-subtitle2'));
  ...
  describe('Display Attribute Eligibility', () => {
    it('clears display attribute when the selected property is removed', async () => {
      ...
      const emailRemove = screen
        .getAllByText('email')
        .find((el) => el.classList.contains('MuiTypography-subtitle2'))
        ?.closest('.MuiPaper-root');
      const removeButton = within(emailRemove as HTMLElement).getByRole('button', {name: /remove property/i});
