...
// erroringAuthCodeStoreProvider is a minimal RuntimeStoreProvider fake used to exercise the
// backend-failure branches of the authorization code store, which a real store cannot be made
// to fail on demand.
type erroringAuthCodeStoreProvider struct {
...
func (suite *AuthorizationCodeStoreTestSuite) TestInsertAuthorizationCode_PutError() {
	store := &authorizationCodeStore{
		storeProvider: &erroringAuthCodeStoreProvider{putErr: errors.New("put failed")},
	}
...
func (suite *AuthorizationCodeStoreTestSuite) TestGetAuthorizationCode_GetError() {
	store := &authorizationCodeStore{
		storeProvider: &erroringAuthCodeStoreProvider{getErr: errors.New("get failed")},
	}
...
func (suite *AuthorizationCodeStoreTestSuite) TestConsumeAuthorizationCode_TakeError() {
	store := &authorizationCodeStore{
		storeProvider: &erroringAuthCodeStoreProvider{takeErr: errors.New("take failed")},
	}
