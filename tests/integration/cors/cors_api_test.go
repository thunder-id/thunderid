...
// CORSIntegrationTestSuite validates the live CORS behavior, sourced entirely from the server-config cors
// section: the declarative read-only layer (cors.yaml fixture) plus the runtime writable layer, with runtime
// PUTs taking effect without a restart. The generic server-config API itself is covered by the serverconfig
// integration package; this suite asserts the enforced Origin matching.
...
// SetupTest clears the cors writable layer so each test starts from the declarative baseline only.
