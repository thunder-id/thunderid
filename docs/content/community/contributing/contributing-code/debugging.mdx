...
**CORS allowed origins** (so the frontend development servers and sample app can talk to the backend) are managed through the server-config `cors` section. Create the declarative resource file `backend/cmd/server/config/resources/server_configs/cors.yaml` and set `server_config.store: composite` in `deployment.yaml` so it loads at boot (the default, `mutable`, does not load declarative resources):
