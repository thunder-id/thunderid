...
func mapSCIMError(svcErr *tidcommon.ServiceError) (httpStatus int, scimType ScimErrorType) {
	...
	case ErrorMissingSchemas.Code,
		...
		ErrorCoreSchemaNotSupported.Code,
	...
	case ErrorInvalidPatchPath.Code, errorCustomAttributeRequiresURN.Code, errorUnrecognizedSchemaURN.Code,
		...
		// 400 noTarget — PATCH remove without a "path".
	case ErrorNoTarget.Code:
		return http.StatusBadRequest, ScimErrorTypeNoTarget

		// 400 mutability — request attempted to change an immutable or read-only (declarative) resource.
	case ErrorImmutableUserType.Code, ErrorMutabilityViolation.Code:
	...
	// 501 — unsupported operation. No scimType: RFC 7644 §3.12 Table 9 defines none for it.
