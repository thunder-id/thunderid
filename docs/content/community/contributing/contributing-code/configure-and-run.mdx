---
title: Configure and Run
description: Learn how to build from source, configure the server, and run the backend and frontend together.
sidebar_position: 2
hide_table_of_contents: false
---

# Configure and Run

## Configure the Server

**Add CORS allowed origins:**

CORS allowed origins are managed through the server-config `cors` section, not `deployment.yaml`. For local development, create the declarative resource file `backend/cmd/server/config/resources/server_configs/cors.yaml` with the Gate and Console origins, and set `server_config.store: composite` in `deployment.yaml` so it loads at boot (the default, `mutable`, does not load declarative resources):

```yaml
name: cors
value:
  allowedOrigins:
    - "https://localhost:5190"
    - "https://localhost:5191"
```

:::tip
This configuration allows the <ProductName /> Gate and <ProductName /> Console applications to communicate with the backend server during development. You can also add or change origins at runtime — without a restart — with `PUT /server-config/cors`.
:::

Open `backend/cmd/server/deployment.yaml` in your favorite text editor and make the following changes:

**Configure the Gate client:**

```yaml
gate_client:
  port: 5190
```

:::tip
This configuration points the backend server to the local <ProductName /> Gate application for authentication.
:::

## Run the Server

### All-in-one (Backend + Frontend together)

Start the <ProductName /> server and apps in development mode.

<CodeGroup>
  <CodeBlock lang="bash" label="Linux/macOS">
    make run
  </CodeBlock>
  <CodeBlock lang="powershell" label="Windows">
    .\build.ps1 run
  </CodeBlock>
</CodeGroup>

:::note
This command will automatically set up your complete development environment:

**What happens:**

- Installs frontend dependencies and builds packages
- Starts the frontend development servers
- Generates TLS certificates (if missing)
- Initializes SQLite databases
- Starts the backend server
- Runs the bootstrap script which creates:
  - The `Console` application with the correct redirect URIs
  - The default admin user (credentials default to `admin` / `admin`; configurable via `ADMIN_USERNAME` / `ADMIN_PASSWORD` environment variables)

**Services that start:**

- **Backend server**: [https://localhost:8090](https://localhost:8090)
- **<ProductName /> Gate** (Login/Register): [https://localhost:5190/gate](https://localhost:5190/gate)
- **<ProductName /> Console** (Admin Console): [https://localhost:5191/console](https://localhost:5191/console)
:::

<details>
<summary>Separate terminals (for debugging independently)</summary>

Use this approach when you need to debug the backend and frontend independently.

**Terminal 1 - Backend:**

<CodeGroup>
  <CodeBlock lang="bash" label="Linux/macOS">
    make run_backend
  </CodeBlock>
  <CodeBlock lang="powershell" label="Windows">
    .\build.ps1 run_backend
  </CodeBlock>
</CodeGroup>

**Terminal 2 - Frontend:**

<CodeGroup>
  <CodeBlock lang="bash" label="Linux/macOS">
    make run_frontend
  </CodeBlock>
  <CodeBlock lang="powershell" label="Windows">
    .\build.ps1 run_frontend
  </CodeBlock>
</CodeGroup>

**Terminal 3 - Seed the initial data (one-time only):**

<CodeGroup>
  <CodeBlock lang="bash" label="Linux/macOS">
    {`SKIP_SECURITY=true API_BASE="https://localhost:8090" \\
  backend/cmd/server/bootstrap/01-default-resources.sh \\
  --console-redirect-uris "https://localhost:5191/console"`}
  </CodeBlock>
  <CodeBlock lang="powershell" label="Windows">
    {`$env:SKIP_SECURITY = "true"
$env:API_BASE = "https://localhost:8090"
& backend\\cmd\\server\\bootstrap\\01-default-resources.ps1 \`
  --console-redirect-uris "https://localhost:5191/console"`}
  </CodeBlock>
</CodeGroup>

This creates the `Console` application and the default admin user (credentials default to `admin` / `admin`). You only need to do this once — the data persists in the SQLite databases.

:::warning
Don't use `./setup.sh` here. That script is designed for the packaged distribution zip and expects a compiled `./thunderid` binary at the project root. From source code, always use the bootstrap script directly as shown above, or use `make run` (Option A) which handles everything for you.
:::

</details>

