...
Activate the user onboarding flow before running this walkthrough. Set `user_onboarding_flow_handle` in `repository/conf/deployment.yaml`, then restart <ProductName /> for the change to take effect:
...
5. Open Sam's invitation email from the inbox at <WayFinderMailUrl /> and open the link. The browser opens a **Complete Your Profile** page.
