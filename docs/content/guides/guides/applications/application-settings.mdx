...
## Configure Login Consent

On the **General** tab under **Login Consent**, control whether users see a consent screen before completing sign-in.

| Setting | Description |
|---------|-------------|
| **Enabled** | When enabled, users see a consent screen showing the application name, logo, and requested scopes before sign-in completes. |
| **Validity Period** | How long (in seconds) a granted consent remains valid. Set to `0` for no expiry. Users are not prompted again while valid consent exists. |
