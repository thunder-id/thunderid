...
import { exchangeAssertion, getAppMetadata, initiateFlow, submitFlowStep } from "../auth/nativeAuthService";
...
// sessionStorage keys for surviving social-login redirects
...
export function NativeAuthPage() {
  ...
  useEffect(() => {
    getAppMetadata().then((meta) => setAppMeta(meta)).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    if (code && isPendingSocial) {
      ...
      const urlHiddenValues = extractUrlHiddenValues(urlParams);
      resumeFlowFromLink(executionId, urlHiddenValues);
    } else {
      startFlow(mode);
    }
  }, []);
  ...
  async function resumeFlowFromLink(executionId, hiddenValues = {}) {
    ...
    try {
      ...
      const response = await submitFlowStep({ executionId, inputs });
      await processResponse(response, hiddenValues);
    } catch (err) {
  ...
  async function startFlow(flowMode) {
    ...
    setFormValues({});
    try {
      const response = await initiateFlow(FLOW_TYPE_MAP[flowMode] || "AUTHENTICATION");
  ...
  async function processResponse(response, pendingHiddenValues = {}) {
    const { flowStatus, type, data = {}, assertion, executionId, challengeToken, failureReason } = response;
    ...
    if (flowStatus === "COMPLETE") {
      if (!assertion) {
        if (mode === "signup") {
          ...
          // Recovery flow completed — send user to sign in.
          navigate("/signin");
        }
        return;
      }
  ...
  async function handleSocialCallback(code) {
    ...
    window.history.replaceState({}, "", window.location.pathname + (window.location.search.replace(/[?&]code=[^&]*/g, "").replace(/[?&]state=[^&]*/g, "") || ""));

    setIsLoading(true);
    setError(null);
    try {
      const response = await submitFlowStep({
        executionId,
  ...
  async function handleFormActionClick(actionRef) {
    await submitFormWithAction(actionRef);
  }
  ...
  return (
    <main className="native-auth-page">
      <div className="native-auth-card">
        ...
        {registrationDone ? (
          <RegistrationSuccess onSignIn={() => switchMode("signin")} />
        ) : (
        <>
        ...
        {isLoading && !flowState && (
          <p className="native-auth-status">Loading…</p>
        )}
        ...
        {flowState && (
          <FlowStep
            ...
            onFormActionClick={handleFormActionClick}
            onActionClick={(actionRef) => submitStep(actionRef, {})}
          />
        )}
        ...
        <div className="native-auth-mode-links">
          {mode === "signin" && (
            <>
              ...
              <span>
                <button type="button" className="native-auth-link-btn" onClick={() => switchMode("recovery")}>
...
function AppBranding({ meta }) {
  const { name, logoUrl } = meta;

  const logo = renderLogo(logoUrl);

  return (
    <div className="native-auth-branding">
      {logo && <div className="native-auth-logo-wrap">{logo}</div>}
      {name && <span className="native-auth-brand-name">{name}</span>}
    </div>
  );
}
...
function renderLogo(logoUrl) {
  ...
  if (logoUrl.startsWith("emoji:")) {
    const emoji = logoUrl.slice("emoji:".length);
    return <span className="native-auth-emoji-logo" aria-hidden="true">{emoji}</span>;
  }
  return <img src={logoUrl} alt="App logo" className="native-auth-img-logo" />;
}

function FlowStep({ flowState, formValues, isLoading, mode, onFieldChange, onFormSubmit, onFormActionClick, onActionClick }) {
  const { inputs = [], actions = [] } = flowState;
  const visibleInputs = inputs.filter((i) => i.type !== "HIDDEN");

  // Decision screen: multiple actions, no visible inputs
  if (actions.length > 1 && visibleInputs.length === 0) {
    return (
        ...
        <div className="native-auth-action-group">
          {actions.map((action) => (
            <button
              key={action.ref}
              type="button"
              className={actionBtnClassFor(action)}
              disabled={isLoading}
  ...
  // Form with inputs + multiple actions (e.g. consent Allow/Deny)
  if (actions.length > 1 && visibleInputs.length > 0) {
    return (
      <div className="native-auth-form-body">
  ...
  // Standard form: inputs + single submit action
  return (
    <form onSubmit={(e) => onFormSubmit(e, actions[0]?.ref)} className="native-auth-form-body">
      ...
      {actions.length === 1 && (
        <button type="submit" className="native-auth-btn-primary" disabled={isLoading}>
          {isLoading ? "Loading…" : submitLabelFor(actions[0]?.ref, visibleInputs, mode)}
        </button>
      )}
    </form>
  );
...
function FlowField({ input, value, onChange, disabled }) {
  const inputType = resolveInputType(input.type);
  ...
  return (
    <label className="native-auth-field-label">
      ...
      <input
        ...
        autoComplete={inputType === "password" ? "current-password" : inputType === "email" ? "email" : "off"}
...
function labelForAction(action, mode = "signin") {
  ...
  // Strip common prefixes and humanize
  const cleaned = ref
    .replace(/^action_role_/, "")
    .replace(/^action_/, "")
    .replace(/_/g, " ")
    .trim();
...
function submitLabelFor(actionRef, visibleInputs, mode) {
  ...
  const hasOtp = visibleInputs.some(
    (i) => i.type === "OTP_INPUT" || i.identifier === "otp"
  );
...
function actionBtnClassFor(action) {
  const ref = action?.ref || "";
