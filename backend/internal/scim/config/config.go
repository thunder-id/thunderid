...
const (
	...
	// ETagSupported indicates that ETag / versioning is not supported per
	// RFC 7644 §3.14. Resource versions are not persisted at the database
	// layer, so there is no way to detect concurrent writes; advertising
	// ETag support without real optimistic concurrency control would be
