...
// the property row header both show the attribute id, and the serialized schema
// omits displayName. Ids are chosen so none is a substring of another.
const {mockAttributes} = vi.hoisted(() => ({
  mockAttributes: [
    {id: 'email', dataType: 'string', required: false, unique: false, credential: false},
    {id: 'username', dataType: 'string', required: true, unique: true, credential: false},
    {id: 'age', dataType: 'number', required: false, unique: false, credential: false},
    {id: 'code', dataType: 'string', required: false, unique: false, credential: false},
    {id: 'password', dataType: 'string', required: false, unique: false, credential: true},
    {id: 'employeeId', dataType: 'number', required: false, unique: false, credential: false},
  ] as Attribute[],
}));
...
async function addAttribute(user: ReturnType<typeof userEvent.setup>, id: string) {
  const panel = within(screen.getByRole('region', {name: /attributes/i}));
...
/**
 * Helper to expand a property row by clicking its header text (the attribute id).
 */
async function expandProperty(user: ReturnType<typeof userEvent.setup>, id: string) {
...
describe('CreateUserTypePage', () => {
  ...
  it('locks the name and type of a library attribute', async () => {
    const user = userEvent.setup();
    renderPage();
    ...
    // The name is locked to the definition for a library-seeded attribute.
    expect(nameInput).toBeDisabled();

    // The property type select stays locked to the definition.
    const typeSelect = screen.getAllByRole('combobox').find((c) => c.getAttribute('aria-disabled') === 'true');
    expect(typeSelect).toBeDefined();
  });

  it('renders unique and credential checkboxes as disabled', async () => {
    const user = userEvent.setup();
    renderPage();
    ...
    expect(screen.getByLabelText(/unique/i)).toBeDisabled();
    expect(screen.getByLabelText(/credential/i)).toBeDisabled();
  ...
  it('allows selecting a display attribute', async () => {
    ...
    // The rows stay collapsed, so the only combobox is the display attribute select.
    const displayAttributeSelect = screen.getByRole('combobox');
  ...
  it('creates schema with string property containing regex pattern', async () => {
    ...
    await addAttribute(user, 'code');
    await expandProperty(user, 'code');
  ...
  it('creates schema with a number property seeded from the attribute', async () => {
    ...
    await expandProperty(user, 'employeeId');
