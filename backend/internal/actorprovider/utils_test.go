...
func (s *UtilsTestSuite) SetupTest() {
	...
	s.provider = Initialize(s.mockInbound, s.mockEntity, managermock.NewAuthnProviderManagerInterfaceMock(s.T()))
}
...
func TestBuildApplication_InboundClientStoreError(t *testing.T) {
	...
	provider := Initialize(mockInbound, mockEntity, managermock.NewAuthnProviderManagerInterfaceMock(t))
