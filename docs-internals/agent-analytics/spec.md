...
structured event for every token issuance and flow step, and since #4931 those events carry the principal
...
Ten published fields carry the meaning the feature depends on:
...
| Which agents carry the load? | Successful issuances grouped by `client_id`, top N |
| How much is done for a person? | Successful issuances split by `is_delegated` |
| How do agents obtain access? | Successful issuances grouped by `grant_type` |
...
`act_type` alone is insufficient because it also matches flow events, where a person signs in *to* an
agent, and the started half of every issuance. Measured against a running deployment, filtering on
`act_type` alone reported 1830 events where 838 token issuances had occurred, an overstatement of 2.2x.
...
| Busiest agents | Which agents carry the workload? | Which would hurt most if they broke, and where to look first |
| Work performed for a person | How much was on someone's behalf? | The consent and compliance exposure of the fleet |
...
renders as a list nobody can act on. Resource identifiers remain available in the detail panel.
...
**Delegation does not imply consent.** On-behalf-of access is an authorization code flow against the
agent, enabled by the agent's actor claim setting: the person signs in and the issued token is theirs
with the agent named as actor. A token exchange reaches the same access without that sign-in. Both were
observed producing identical events differing only in grant type, so grant type is the only field
separating a consented sign-in from an exchange.

**Span timing on the OpenTelemetry path is not operation timing.** The subscriber starts each span at the
event timestamp and ends it at the current clock, so span duration measures delivery latency. Nothing
...
The file sink's rotation settings are implemented in the adapter but were never exposed, so the log grows
without bound and documented retention settings have no effect. This specification exposes them.
...
- **AC7.1:** Given a product that ingests log files, when the file output is enabled, then the published
  events are complete, durable, and carry the operation's true duration.
