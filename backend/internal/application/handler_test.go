...
func (suite *HandlerTestSuite) TestHandleApplicationListRequest_ForwardsFilter() {
	...
	mockService.On("GetApplicationList", mock.Anything, mock.Anything, mock.Anything, matchFilter).
		Return(expectedList, nil)
...
func (suite *HandlerTestSuite) TestHandleApplicationListRequest_ForwardsPagination() {
	mockService := NewApplicationServiceInterfaceMock(suite.T())
	handler := newApplicationHandler(mockService)

	mockService.On("GetApplicationList", mock.Anything, 10, 20, mock.Anything).
		Return(&model.ApplicationListResponse{}, nil)

	req := httptest.NewRequest(http.MethodGet, "/applications?limit=10&offset=20", nil)
	w := httptest.NewRecorder()
...
func (suite *HandlerTestSuite) TestHandleApplicationListRequest_DefaultsLimitWhenAbsent() {
	mockService := NewApplicationServiceInterfaceMock(suite.T())
	handler := newApplicationHandler(mockService)

	mockService.On("GetApplicationList", mock.Anything, serverconst.DefaultPageSize, 0, mock.Anything).
		Return(&model.ApplicationListResponse{}, nil)
...
func (suite *HandlerTestSuite) TestHandleApplicationListRequest_InvalidPaginationReturns400() {
	for _, url := range []string{
		"/applications?limit=abc",
		"/applications?limit=0",
		"/applications?limit=-5",
		"/applications?offset=-1",
		"/applications?offset=xyz",
	} {
		mockService := NewApplicationServiceInterfaceMock(suite.T())
		handler := newApplicationHandler(mockService)
		...
		assert.Equal(suite.T(), http.StatusBadRequest, w.Code, "url=%s", url)
	}
}
