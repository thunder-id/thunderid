...
By default, <ProductName /> only accepts tokens it issued itself. If your users authenticate against an external authorization server — another <ProductName /> instance, an enterprise identity provider, or any standard OIDC provider — those users cannot call <ProductName />'s own APIs or sign in to the <ProductName /> Console. This requires <ProductName /> to validate their tokens.
  ...
  server:
    security:
      trustedIssuer:
        ...
        audience: "https://{{productSlug}}.example.com"
        requiredClaims:
          - claim: "ouId"
            value: "tenant-123"
```
...
- [Token Exchange with External Identity Providers](../../guides/identity-providers/token-exchange-idp) — Exchange a token from an external identity provider for a <ProductName />-issued access token.
- [Resource Indicators](../../guides/resource-indicators) — Use the `resource` parameter to set the `aud` claim on tokens issued by the central authorization server.
- [Resource Servers](../../guides/resource-servers) — Define resource servers with identifiers and permissions.
