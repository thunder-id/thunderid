...
> **CORS allowed origins** are configured through the server-config `cors` section, not `deployment.yaml`. Create the declarative resource `config/resources/server_configs/cors.yaml` in the image and set `server_config.store: composite` in `deployment.yaml` so it loads at boot, or apply origins at runtime — without a restart — via `PUT /server-config/cors`:
>
> ```yaml
> name: cors
> value:
...
Ensure your full origin (host + port) is listed in the server-config `cors` section. Add it to the declarative resource `config/resources/server_configs/cors.yaml` (with `server_config.store: composite` set so it loads at boot), or apply it at runtime via `PUT /server-config/cors`.
