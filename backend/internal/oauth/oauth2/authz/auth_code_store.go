...
func (acs *authorizationCodeStore) InsertAuthorizationCode(
	...
	ttlSeconds := int64(authzCode.ExpiryTime.Sub(time.Now().UTC()).Seconds())
	err = acs.storeProvider.Put(ctx, providers.NamespaceAuthzCode, authzCode.Code, data, ttlSeconds)
