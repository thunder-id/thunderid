...
func stripCredentialFields(
	ctx context.Context, logger log.Logger, attrs json.RawMessage, credentialKeys map[string]struct{},
) json.RawMessage {
...
func buildSCIMUserResource(
	ctx context.Context, logger log.Logger, u user.User, extensionURN, baseURL string,
	...
	scimUser := SCIMUser{
		...
		Meta: scim.SCIMMeta{
			ResourceType: "User",
			Location:     location,
			Version:      scim.GenerateVersion(userVersionState(u)),
		},
	}

	if len(u.Attributes) > 0 {
		scimUser.Attributes = stripCredentialFields(ctx, logger, u.Attributes, credKeys)
		if includeCoreAttrs {
...
func resolveAttributePath(attr, extensionURN string) (name string, coreOnly, extensionOnly bool) {
	...
	if strings.HasPrefix(attr, scim.SCIMCoreUserSchemaURN+":") {
		return attr[len(scim.SCIMCoreUserSchemaURN)+1:], true, false
	}
