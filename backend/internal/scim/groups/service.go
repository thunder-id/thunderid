...
func (s *scimGroupsService) ReplaceGroup(ctx context.Context, groupID, displayName string,
	...
	if trimmed := strings.TrimSpace(ifMatch); trimmed != "" {
		...
		if svcErr != nil {
			s.logger.Error(ctx, "SCIM ReplaceGroup: failed to fetch group members",
				log.String("groupID", groupID), log.Any("error", svcErr))
			return nil, mapGroupServiceErrorToSCIM(svcErr)
		}
		g.Members = existingMembers
		if svcErr := scim.CheckIfMatch(trimmed, scim.GenerateVersion(groupVersionState(*g))); svcErr != nil {
			return nil, svcErr
		}
	}
	...
	if svcErr != nil {
		s.logger.Error(ctx, "SCIM ReplaceGroup: failed to fetch existing members",
			log.String("groupID", groupID), log.Any("error", svcErr))
		return nil, mapGroupServiceErrorToSCIM(svcErr)
	}
	if len(existingMembers) > 0 {
		if _, svcErr = s.groupService.RemoveGroupMembers(ctx, groupID, existingMembers); svcErr != nil {
			s.logger.Error(ctx, "SCIM ReplaceGroup: failed to remove existing members",
				log.String("groupID", groupID), log.Any("error", svcErr))
			return nil, mapGroupServiceErrorToSCIM(svcErr)
		}
	}
	if len(thunderIDMembers) > 0 {
		if _, svcErr = s.groupService.AddGroupMembers(ctx, groupID, thunderIDMembers); svcErr != nil {
			s.logger.Error(ctx, "SCIM ReplaceGroup: failed to add new members",
				log.String("groupID", groupID), log.Any("error", svcErr))
			return nil, mapGroupServiceErrorToSCIM(svcErr)
		}
	}
...
func (s *scimGroupsService) PatchGroup(ctx context.Context, groupID string, actions []SCIMGroupPatchAction,
	...
	if trimmed := strings.TrimSpace(ifMatch); trimmed != "" {
		existingMembers, svcErr := s.fetchAllGroupMembers(ctx, groupID, true)
		if svcErr != nil {
			s.logger.Error(ctx, "SCIM PatchGroup: failed to fetch group members",
				log.String("groupID", groupID), log.Any("error", svcErr))
			return nil, mapGroupServiceErrorToSCIM(svcErr)
		}
		g.Members = existingMembers
...
func (s *scimGroupsService) DeleteGroup(ctx context.Context, groupID string, ifMatch string,
	...
	if trimmed := strings.TrimSpace(ifMatch); trimmed != "" {
		existingMembers, svcErr := s.fetchAllGroupMembers(ctx, groupID, true)
		if svcErr != nil {
			s.logger.Error(ctx, "SCIM DeleteGroup: failed to fetch group members",
				log.String("groupID", groupID), log.Any("error", svcErr))
			return mapGroupServiceErrorToSCIM(svcErr)
		}
		g.Members = existingMembers
...
func (s *scimGroupsService) applyMembersPatch(ctx context.Context, groupID string,
	...
	case scimPatchOpReplace:
		...
		if svcErr != nil {
			s.logger.Error(ctx, "SCIM PatchGroup: failed to fetch existing members for replace",
				log.String("groupID", groupID), log.Any("error", svcErr))
			return mapGroupServiceErrorToSCIM(svcErr)
		}
		if len(existing) > 0 {
			if _, svcErr := s.groupService.RemoveGroupMembers(ctx, groupID, existing); svcErr != nil {
				s.logger.Error(ctx, "SCIM PatchGroup: failed to remove existing members for replace",
					log.String("groupID", groupID), log.Any("error", svcErr))
				return mapGroupServiceErrorToSCIM(svcErr)
			}
		}
		if len(thunderIDMembers) > 0 {
			if _, svcErr := s.groupService.AddGroupMembers(ctx, groupID, thunderIDMembers); svcErr != nil {
				s.logger.Error(ctx, "SCIM PatchGroup: failed to add new members for replace",
					log.String("groupID", groupID), log.Any("error", svcErr))
				return mapGroupServiceErrorToSCIM(svcErr)
			}
		}
...
// Any client-supplied "display" is discarded: ThunderID always derives it from the
// referenced resource's own display attribute on read (see fetchAllGroupMembers), so a
// client-supplied value would never be echoed back and must not be stored.
