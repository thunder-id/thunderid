...
This flow is built using OAuth2 authorization code followed by a token exchange that converts the user's token into an on-behalf-of (OBO) token carrying the agent as actor. See [Act on Behalf of a User](../../../guides/agents/authentication/on-behalf-of-user).
...
This flow is built using OAuth2 token exchange ([RFC 8693](https://datatracker.ietf.org/doc/html/rfc8693)).
