...
Controls which browser origins may call the <ProductName /> API. Allowed origins are stored in the server-config `cors` section — not in `deployment.yaml`. None are allowed by default, so a browser app on a different origin cannot reach the API until you add its origin.
...
Add origins in one of two ways:

- **Declarative** — persistent and loaded at startup. Create `config/resources/server_configs/cors.yaml` and set `server_config.store: composite` in `deployment.yaml` so it loads (the default `mutable` mode does not load declarative files), then restart:
  ```yaml
  name: cors
  value:
    allowedOrigins:
  ...
  ```
- **Runtime** — applied immediately without a restart, merged with the declarative baseline:
  ```http
...
Literal origins are matched after the request `Origin` is normalized. Scheme and host are case-insensitive, a trailing host dot is ignored, and IPv6 hosts use bracket form. Internationalized domain names are compared in ASCII form, and ports must match exactly — list each port separately. Methods, headers, credentials, and the preflight cache TTL are fixed per route in code; only the origin list is configurable, and preflight responses default to a 600-second `Access-Control-Max-Age`.

:::note
Anchor every regex with `^` and `$` and escape literal dots. An unanchored pattern produces a startup warning because it allows partial matches, and an invalid pattern stops the server at startup.
:::
