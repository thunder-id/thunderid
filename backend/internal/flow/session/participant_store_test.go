...
func (s *ParticipantStoreTestSuite) TestListBySessionIDs_ChunksAtTheParameterCap() {
	...
	// First call carries a full chunk of ids plus the deployment id. Its row ("sess-5") sorts after the
	// second chunk's row ("sess-200"), so the combined result must be re-sorted.
	...
	s.NoError(err)
	s.Require().Len(got, 3)
	s.Equal(ids[total-1], got[0].SessionID, "sorted by session id across chunks")
	s.Equal(ids[5], got[1].SessionID)
	s.Equal("app-0", got[1].AppID, "then by join time within a session")
	s.Equal("app-1", got[2].AppID)
