...
| **Claim sourcing** | Claims are resolved from the user's profile attributes. Missing attributes are silently omitted from the issued credential. Every configured claim is advertised in the metadata `claims` array, each with its own `path`; a claim carries a `display` entry only when it has a `displayName`. |
...
| **Claims** | Each entry maps a user profile attribute name to an optional wallet display name. The attribute value is sourced from the user's profile at issuance. Every entry is advertised in the metadata `claims` array, and one with a display name also carries a `display` entry. |
