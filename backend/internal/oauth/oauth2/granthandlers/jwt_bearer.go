...
func (h *jwtBearerGrantHandler) HandleGrant(ctx context.Context, tokenRequest *model.TokenRequest,
	...
	*model.TokenResponseDTO, *model.ErrorResponse) {
	...
	// When authorization mapping (rule-based or direct) is configured, the assertion's own scope claim
	// is skipped; requested scopes are only candidates, narrowed later by RS definition and
	// ApplyMappedAuthorization (below).
