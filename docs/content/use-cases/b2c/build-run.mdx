...
application on the same flow reads it. [Build the Flows](../build-flows#build-the-sign-out-flow) shows how it
unwinds.
...
- One sign-in survives a return trip, runs out on two clocks, and reaches every application you put on the same flow.
- One sign-out ends it at <ProductName />, not just in the application the customer clicked in.
