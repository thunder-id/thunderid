...
// IsBlanket reports whether the scope reaches a whole family rather than named organization units.
// Blanket policies may only ever be narrowed, and only by exclusion.
func (s TargetScope) IsBlanket() bool {
	return s == TargetScopeAllOUs || s == TargetScopeAllRoots || s == TargetScopeAllChildren
}
...
type Target struct {
	...
	Scope TargetScope
	// OUID is the named organization unit; empty for the blanket scopes.
	OUID string
}
...
// AsRequest returns the policy in the shape that recreates it, so an API response can be edited and
// sent straight back as an update body, and an export replays without a second conversion.
func (p Policy) AsRequest() PolicyRequest {
	return requestFromPolicy(p)
}
...
type TargetOUScope struct {
	...
	AllChildren bool `json:"allChildren,omitempty" yaml:"allChildren,omitempty"`
	// OUIDs names direct children of the initiator, each deciding whether its subtree comes too.
	OUIDs []TargetEntry `json:"ouIds,omitempty" yaml:"ouIds,omitempty"`
...
func (s TargetOUScope) Mode() (blanket, root, children bool, valid bool) {
	...
	children = s.AllChildren || len(s.OUIDs) > 0
