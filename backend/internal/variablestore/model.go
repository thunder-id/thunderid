...
type Secret struct {
	Name string `json:"name"`
	// Exists is always true on anything returned: a name with nothing stored is absent from the
	// result rather than listed as not existing. It states in the body what membership already
	// implies, so a client reads the fact rather than inferring it from the entry being there.
