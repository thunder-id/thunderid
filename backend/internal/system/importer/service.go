...
type authZENPDPAdapter interface {
	CreateAuthZENPDP(ctx context.Context, connection authzenpdp.AuthZENPDPConnection) error
	GetAuthZENPDP(ctx context.Context, id string) (*authzenpdp.AuthZENPDPConnection, error)
	UpdateAuthZENPDP(ctx context.Context, id string, connection authzenpdp.AuthZENPDPConnection) error
}
...
func (s *importService) importConnectionAuthZENPDP(
	...
	if options.IsUpsertEnabled() && req.ID != "" {
		existing, err := s.authZENPDPService.GetAuthZENPDP(ctx, req.ID)
		if err != nil {
		...
		if existing != nil {
			...
			if err := s.authZENPDPService.UpdateAuthZENPDP(ctx, req.ID, *req); err != nil {
				return authZENPDPImportError(req, operationUpdate, err)
			}
			return successOutcome(resourceTypeConnection, req.ID, req.Name, operationUpdate)
		}
	...
	if err := s.authZENPDPService.CreateAuthZENPDP(ctx, *req); err != nil {
		return authZENPDPImportError(req, operationCreate, err)
	}
	return successOutcome(resourceTypeConnection, req.ID, req.Name, operationCreate)
}

// authZENPDPImportError converts an AuthZEN PDP connection error into an import outcome.
