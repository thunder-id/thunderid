...
func TestIntersectIsMonotone(t *testing.T) {
	...
	// Validate accepts.
	t.Run("an editable value is clamped to the bound it is folded with", func(t *testing.T) {
		got := c.Intersect([]Rule{
		...
		assert.Equal(t, []string{"b"}, *got.Value, "the value keeps only what the bound allows")
		require.NotNil(t, got.AllowedValues)
		assert.Equal(t, []string{"b", "c"}, *got.AllowedValues)
		assert.NoError(t, c.Validate(got), "the fold must not produce a rule it would itself reject")
	})

	// Nothing survives when the two are disjoint, which is the honest answer rather than a value
	// standing outside its own menu.
	t.Run("a disjoint bound empties the value", func(t *testing.T) {
		got := c.Intersect([]Rule{
		...
		assert.NoError(t, c.Validate(got))
	})
