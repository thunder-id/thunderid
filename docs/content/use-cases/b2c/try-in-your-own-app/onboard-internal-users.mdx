...
:::note Prerequisites
Configure SMTP so that <ProductName /> can send invitation emails. See [Email Configuration](../../../../deployment/configuration#email-configuration), then restart <ProductName /> for the changes to take effect.
:::
...
Build a `USER_ONBOARDING` flow that sends the invitation email and provisions the invitee with the correct role on acceptance. See [Build a Flow](../../../../guides/flows/build-a-flow).
