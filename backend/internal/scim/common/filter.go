...
type FilterAttrRules struct {
	...
	// ValidatePrefixedAttr checks that a schema URN prefix (with trailing colon) names a known
	// schema and that attr belongs to it. Optional; nil skips the check.
	ValidatePrefixedAttr func(prefix, attr string) *tidcommon.ServiceError
}

// filterServiceError carries a ServiceError raised while parsing so it is returned as-is
// instead of being rewrapped as a filter syntax error.
type filterServiceError struct {
	svcErr *tidcommon.ServiceError
}

func (e *filterServiceError) Error() string { return e.svcErr.Code }
...
func ParseSCIMFilterForEq(filterStr string, rules FilterAttrRules) (map[string]interface{}, *tidcommon.ServiceError) {
	...
	if err != nil {
		var wrapped *filterServiceError
		if errors.As(err, &wrapped) {
			return nil, wrapped.svcErr
		}
		return nil, newInvalidFilterSyntaxError(err.Error())
	}
...
func (p *scimFilterParser) leaf() (scimFilterNode, error) {
	...
	if prefix != "" && p.rules.ValidatePrefixedAttr != nil {
		if svcErr := p.rules.ValidatePrefixedAttr(prefix, attr); svcErr != nil {
			return nil, &filterServiceError{svcErr: svcErr}
		}
	}
