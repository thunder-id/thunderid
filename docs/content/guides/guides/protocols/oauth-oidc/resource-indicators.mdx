...
| No `resource` parameter | 0 | <ProductName /> discovers which registered resource servers own the granted scopes and includes their identifiers. Falls back to the `client_id` if no resource server matches. |
