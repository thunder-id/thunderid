...
// ----- GetEntityTypeUsages Tests -----

func TestGetEntityTypeUsages_MissingSchemaID(t *testing.T) {
	...
	result, svcErr := service.GetEntityTypeUsages(context.Background(), TypeCategoryUser, "")
	require.Nil(t, result)
	require.NotNil(t, svcErr)
	require.Equal(t, ErrorInvalidUserTypeRequest.Code, svcErr.Code)
}

func TestGetEntityTypeUsages_NotFound(t *testing.T) {
	testConfig := &config.Config{DeclarativeResources: config.DeclarativeResources{Enabled: false}}
	config.ResetServerRuntime()
	require.NoError(t, config.InitializeServerRuntime("/tmp/test", testConfig))
	defer config.ResetServerRuntime()
	...
	storeMock := newEntityTypeStoreInterfaceMock(t)
	storeMock.On("GetEntityTypeByID", mock.Anything, mock.Anything, "missing").
		Return(EntityType{}, ErrEntityTypeNotFound).Once()
	...
	result, svcErr := service.GetEntityTypeUsages(context.Background(), TypeCategoryUser, "missing")
	require.Nil(t, result)
	require.NotNil(t, svcErr)
	require.Equal(t, ErrorUserTypeNotFound.Code, svcErr.Code)
}

func TestGetEntityTypeUsages_NilDependencyRegistry_ReturnsUnknown(t *testing.T) {
	const schemaID = "schema-usages-1"
	testConfig := &config.Config{DeclarativeResources: config.DeclarativeResources{Enabled: false}}
	config.ResetServerRuntime()
	require.NoError(t, config.InitializeServerRuntime("/tmp/test", testConfig))
	defer config.ResetServerRuntime()
	...
	storeMock := newEntityTypeStoreInterfaceMock(t)
	storeMock.On("GetEntityTypeByID", mock.Anything, mock.Anything, schemaID).
		Return(EntityType{ID: schemaID, OUID: testOUID1}, nil).Once()
	...
	result, svcErr := service.GetEntityTypeUsages(context.Background(), TypeCategoryUser, schemaID)
	require.Nil(t, svcErr)
	require.NotNil(t, result)
...
func TestGetEntityTypeUsages_ReturnsBlockingUsages(t *testing.T) {
	const schemaID = "schema-usages-2"
	testConfig := &config.Config{DeclarativeResources: config.DeclarativeResources{Enabled: false}}
	config.ResetServerRuntime()
	require.NoError(t, config.InitializeServerRuntime("/tmp/test", testConfig))
	defer config.ResetServerRuntime()
