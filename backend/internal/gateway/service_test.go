...
func TestRegisterStoresTheCredentialEncrypted(t *testing.T) {
	...
	// It must still be recoverable, or nothing could ever authenticate to the gateway.
...
// One gateway registers once. Two rows for the same gateway would be applied to
...
func TestRegisterRefusesAnAddressAlreadyRegistered(t *testing.T) {
	...
	if svcErr == nil {
		t.Fatal("expected the second registration of a gateway to be refused")
	}
	if svcErr.Code != ErrorGatewayAlreadyRegistered.Code {
		t.Fatalf("expected %s, got %s", ErrorGatewayAlreadyRegistered.Code, svcErr.Code)
	}
}

// A registration is incomplete without the three things needed to reach the gateway.
func TestRegisterRequiresWhatItTakesToReachTheGateway(t *testing.T) {
	cmodels.SetConfigCryptoProvider(reversingCrypto{})
	t.Cleanup(func() { cmodels.SetConfigCryptoProvider(nil) })
...
// hand every operator of this control plane the keys to every gateway it administers.
...
// A declared gateway may not take a gateway another gateway already owns. Without this the unique
...
// Re-reading the same declaration keeps its own gateway, which is what makes the load idempotent.
...
// a gateway safe: its gateway keeps answering to the credential it already holds.
