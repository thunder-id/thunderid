...
// buildUnresolvedSCIMUser returns a bare resource (id, core schema, meta) for a user whose type
// schema cannot be resolved. It carries no attributes, since none can be filtered against a schema.
func buildUnresolvedSCIMUser(userID, baseURL string) SCIMUser {
	return SCIMUser{
		ID:      userID,
		Schemas: []string{scim.SCIMCoreUserSchemaURN},
		Meta: scim.SCIMMeta{
			ResourceType: "User",
			Location:     fmt.Sprintf("%s%s/Users/%s", baseURL, scim.SCIMBasePath, userID),
		},
	}
}
