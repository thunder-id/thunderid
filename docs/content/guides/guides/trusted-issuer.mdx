...
| Field | Description |
|-------|-------------|
| `baseUrl` | Fully-qualified URL of the external authorization server. The console redirects users here for login. |
| `clientId` | OAuth client ID registered on the external authorization server for this <ProductName /> instance's console. |
...
| `signInOptions.resource` | The resource indicator sent in the authorization request so the issued token's `aud` points at this <ProductName /> instance. Set to this instance's public URL. |
| `preferences.resolveFromMeta` | Set to `false` for generic OIDC providers to skip the <ProductName />-specific flow-metadata bootstrap call that would fail against a generic provider. |
| `preferences.theme.inheritFromBranding` | Set to `false` for generic OIDC providers to skip the branding-preferences call. |
| `sendCookiesInRequests` | Set to `false` for generic OIDC providers. External authorization servers typically respond with `access-control-allow-credentials: false`, so credentialed token requests are blocked by CORS. Setting this to `false` switches to `credentials: 'same-origin'`, which is CORS-safe for cross-origin requests. |

When an `sdk` block with `baseUrl` and `clientId` is present, the console delegates login to the external authorization server. When omitted, the console authenticates against this <ProductName /> instance normally.
...
To configure the console portal instead of editing `apps/console/config.js` directly, set `configuration.consoleClient.sdk` in the chart values. The fields mirror those described in [Console Portal Configuration](#console-portal-configuration):
...
configuration:
  consoleClient:
    sdk:
      ...
      clientId: "FEDERATED_CONSOLE"
      scopes: "['openid', 'profile', 'email', 'group', 'ou', 'system', 'system:user', 'system:group', 'system:ou:view', 'system:usertype:view']"
      signInOptions: {resource: "https://thunder.example.com"}
      preferences: {resolveFromMeta: false, theme: {inheritFromBranding: false}}
      sendCookiesInRequests: false
```
...
| Console never redirects to the external authorization server | `consoleClient.sdk.baseUrl` not set, or still points at this <ProductName /> instance | Verify the `consoleClient.sdk` block and re-render the chart. |
