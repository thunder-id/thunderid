...
An administrative caller retrieves a client's currently registered metadata. The endpoint checks the system permission, then the service resolves the client and returns its metadata. The client secret is omitted because it is stored as a hash and cannot be recovered from what is stored.
...
| 3 | [Information Disclosure] | The client secret is returned on read and captured from logs, proxies or client-side storage. | [No] | The secret is stored as a hash, which the platform can verify against but cannot reverse, so the response cannot contain it. This is also a deliberate deviation from the RFC 7592 example response, recorded in the specification. |

#### [02]: Update client registration

**Description**

An administrative caller replaces a client's registered metadata. The endpoint authorizes as in [01], then the service resolves the client and carries the client identifier, client secret, owning organization unit and application type forward from the existing registration before applying the replacement. A `client_id` in the body that contradicts the path is rejected, and a `client_secret` in the body is verified against the stored hash and rejected on mismatch. Localized variants of the human-readable fields are replaced rather than merged, so a variant the request omits does not survive the update.
...
| 5 | [Tampering] | An update silently reissues the client identifier or client secret, breaking a working integration or invalidating credentials the client still holds. | [No] | The existing identifier and organization unit are set explicitly on the replacement, and no secret is supplied to storage, which is what preserves the stored one. A `client_secret` in the request body is only ever compared against the stored hash, never written, so no submitted value can become the client's credential. The application type is left empty so the immutable existing type is inherited. Covered by a unit test asserting client identity is preserved across an update. |
| 6 | [Tampering] | Because the update is a full replacement per RFC 7592, a caller that omits a field it did not intend to change silently wipes it, for example dropping a redirect URI or a set of contacts from a working registration. | [Yes] | Residual by design. RFC 7592 specifies replacement rather than merge, and ThunderID follows it: fields absent from the request are not retained, except for the identity fields listed above and the client name, which is inherited when omitted. Bounded because the caller is administrative and the previous value is recoverable only from a backup, not from logs. The public documentation states explicitly that an update replaces metadata in full. What is not bounded is reconstructing an erroneous overwrite afterwards, tracked below as the audit residual. |
| 6a | [Tampering] | An update silently keeps a localized variant the caller intended to remove, so a name, logo, terms or policy URI the operator believes it deleted keeps rendering to users in that language. A stale localized `logo_uri`, `tos_uri` or `policy_uri` can point at a host the operator no longer controls. | [No] | The i18n store upserts, so writing the new variants alone would merge them into the stored ones. The update therefore clears the stored variants of every localizable field the request supplies a value for, base or localized, before writing the new set, which makes variants follow the same replace semantics as the rest of the metadata. Clearing is scoped to the fields the request mentions, so an update that says nothing about a localizable field leaves its variants alone rather than silently discarding every translation the client had. Covered by unit tests asserting an omitted variant is cleared and that an unmentioned field is not. |
| 7 | [Denial of Service] | A caller submits repeated updates, or a large metadata payload, to exhaust server resources. | [Yes] | Residual. There is no rate limit specific to this endpoint. Bounded because the caller must hold the system permission, which narrows the surface to administrative callers, and each request replaces a single record. ThunderID applies no request rate limiting at any endpoint, so this is a platform-wide deployment concern rather than a property of this feature: operators should rate limit at the edge. |
...
| 10 | [Information Disclosure] | An administrative caller recovers a client secret it does not hold by submitting candidate secrets to the update endpoint and watching which one is accepted, since a matching secret is answered differently from a mismatched one. | [Yes] | Residual, and accepted. The check is required by RFC 7592 section 2.2, and any conforming implementation of it is distinguishable in this way. Bounded on three counts. The caller must already hold the system permission, which authorizes rewriting or deleting the registration outright, so guessing the secret yields less than the access it presupposes. The secret is stored hashed with a work factor, so each guess costs a verification rather than a comparison against recovered plaintext. And a guess is a full update request against one client, which is neither cheap nor quiet. Operators should rate limit at the edge, as for threat 7. |
| 11 | [Spoofing] | A submitted `client_secret` is treated as authentication, letting a caller that knows a client's secret but lacks the system permission perform an update. | [No] | The permission check runs first and is the only authorization decision at the endpoint. The secret comparison is metadata validation, not authentication: it can cause a request to be rejected, never to be authorized. A caller without the system permission is rejected with `401` before any secret is examined. |
...
| --- | --- | --- | --- |
| 1 | Are all inputs and outputs validated (syntactic and semantic)? | [Yes] | The client identifier comes from the request path and is used only as a lookup key. Submitted metadata passes the same validation as registration. A `client_id` in the body that contradicts the path is rejected, as is a `client_secret` that does not match the issued secret. |
...
| 16 | Are sensitive values such as credentials and keys stored in a secret store or vault? | [Yes] | This feature introduces no credential of its own. The client secret is stored as a hash by the existing credential path. This feature verifies a submitted secret through that path and never reads or writes a credential. |
...
| --- | --- | --- | --- |
| Administrative credential reaches every registration | One administrative credential governs every dynamically registered client. A caller that obtains an administrative access token can rewrite or delete all of them, with nothing scoping the damage to a single registration. | Accepted. The trade-off of scoping management to administrative callers, taken in exchange for issuing no durable per-client credential at all. Containment relies on the platform's existing protection, lifetime and revocation of administrative tokens. | Close it further with a scoped permission that names the registrations a caller may manage, or by reintroducing a per-client credential with individual revocation. (threat 8) |
| Configuration changes are not attributable from logs | There is no dedicated audit record for a registration change or deletion, and no before-and-after diff, so a change cannot be attributed to a particular administrative caller from logs alone. This matters more now that every change is administrative. | Open. Requests appear in access logs with a trace identifier, and server errors are logged with an error code and the operation, but no audit record names the acting principal. | Add an audit record naming the acting principal for a registration change or deletion, and record the previous value so an erroneous overwrite can be reconstructed. (threats 6 and 14, checklist items 13 and 14) |
...
| Version | Date | Change |
|---|---|---|
| 1.0 | 2026-09-21 | Initial threat model. |
