...
func newCaptchaInterceptor(flowFactory core.FlowFactoryInterface,
	captchaService captcha.CaptchaServiceInterface) *captchaInterceptor {
...
// Execute validates the captcha token carried in the node's user inputs. A missing token passes
// through, leaving presence enforcement to the prompt node's required-input declaration; an invalid
...
func (c *captchaInterceptor) Execute(ctx *core.InterceptorContext) (*common.InterceptorResponse, error) {
	if ctx.Mode != providers.InterceptorModePreNode {
		return &common.InterceptorResponse{
			Status: common.InterceptorStatusFailure,
		}, nil
	}

	token := ctx.UserInputs[c.resolveFieldKey(ctx)]
	if token == "" {
		return &common.InterceptorResponse{
...
// resolveFieldKey returns the user-input field name carrying the captcha token, read from the
// interceptor's fieldKey property and falling back to the default when it is unset.
func (c *captchaInterceptor) resolveFieldKey(ctx *core.InterceptorContext) string {
	if fieldKey, ok := ctx.Properties[propertyKeyFieldKey].(string); ok && fieldKey != "" {
		return fieldKey
	}
	return defaultCaptchaFieldKey
}
