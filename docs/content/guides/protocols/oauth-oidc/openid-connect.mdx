...
| `sid` | Identifier of the SSO session the authentication belongs to. Applications that share one session receive the same value, and it stays the same across refresh. Omitted when the login flow established no session. Never present in access tokens |
...
The ID Token's `auth_time` claim reports that same moment, taken from the session rather than from the clock at the time the token is built. That applies to the ID Token issued by the login itself, not only to later authorizations, so a login and every authorization that reuses its session report an identical `auth_time`. An authorization that re-authenticated, whether because of `prompt=login` or a `max_age` the previous authentication no longer satisfied, carries a later one.
