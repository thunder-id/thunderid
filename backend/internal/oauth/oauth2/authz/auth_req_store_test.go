...
// erroringAuthReqStoreProvider is a minimal RuntimeStoreProvider fake used to exercise the
// backend-failure branches of AddRequest/GetRequest/ClearRequest, which a real store cannot be
// made to fail on demand.
type erroringAuthReqStoreProvider struct {
...
func (suite *AuthorizationRequestStoreTestSuite) TestAddRequest_PutError() {
	store := &authorizationRequestStore{
		storeProvider:  &erroringAuthReqStoreProvider{putErr: errors.New("put failed")},
		validityPeriod: 10 * time.Minute,
	}
...
func (suite *AuthorizationRequestStoreTestSuite) TestGetRequest_GetError() {
	store := &authorizationRequestStore{
		storeProvider: &erroringAuthReqStoreProvider{getErr: errors.New("get failed")},
	}
...
func (suite *AuthorizationRequestStoreTestSuite) TestClearRequest_DeleteError() {
	store := &authorizationRequestStore{
		storeProvider: &erroringAuthReqStoreProvider{deleteErr: errors.New("delete failed")},
	}
