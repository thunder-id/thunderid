...
func parseAndValidateSCIMUserRequest(body []byte, urnPrefix string) (*SCIMUserPayload, *tidcommon.ServiceError) {
...
func extractCoreAttrs(raw map[string]json.RawMessage, extensionURN string) map[string]json.RawMessage {
	...
	for k, v := range raw {
		if strings.EqualFold(k, "schemas") || slices.ContainsFunc(ignoredCommonAttrs, func(a string) bool {
			return strings.EqualFold(k, a)
		}) {
			continue
		}
...
// validateCoreUserSchemaDeclared ensures the core User schema URN is declared in schemas
//	when core attributes are present.
