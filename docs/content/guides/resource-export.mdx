...
## Organization Unit Timestamps

Exported organization units carry the time the unit was created and the time it was last modified:

| Field | Description |
|---|---|
| `createdAt` | Time the organization unit was created. |
| `updatedAt` | Time the organization unit was last modified. |

Both values are converted to UTC and serialized as RFC 3339:
...
createdAt: 2026-09-08T09:16:26Z
updatedAt: 2026-09-09T10:30:00Z
```

Sub-second precision is retained when the stored value carries it, as in `2026-09-08T09:16:26.123456789Z`. Trailing zeros are dropped, so a whole-second value carries no fractional part.

Both fields are omitted when no timestamps are recorded for the unit. Organization units defined declaratively in YAML files record no timestamps.
