...
type AuthenticationServiceInterface interface {
	...
	SendOTP(ctx context.Context, senderID string, channel notifcommon.ChannelType,
		recipient, priorSessionToken string) (
...
func (as *authenticationService) SendOTP(ctx context.Context, senderID string, channel notifcommon.ChannelType,
	...
	var recipientAttr string
	switch channel {
	case notifcommon.ChannelTypeSMS:
		recipientAttr = "mobile_number"
	default:
		logger.Error(ctx, "Unsupported notification channel for OTP", log.String("channel", string(channel)))
		return "", &tidcommon.InternalServerError
	}

	sessionToken, otpValue, expirySeconds, svcErr := as.otpService.GenerateOTP(
		ctx, recipient, recipientAttr, nil, priorSessionToken)
	if svcErr != nil {
