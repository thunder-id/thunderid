...
func (s *stubProvider) GetResourceUsages(
	_ context.Context, _, _ string) ([]ResourceUsage, error) {
	return s.usages, s.err
}

func TestGetUsagesAggregatesAcrossProviders(t *testing.T) {
	reg := NewRegistry()
	reg.RegisterProvider(&stubProvider{usages: []ResourceUsage{
	...
	resp, err := reg.GetUsages(context.Background(), "theme", "t1")
...
func TestGetUsagesProviderErrorReturnsUnknown(t *testing.T) {
	reg := NewRegistry()
	reg.RegisterProvider(&stubProvider{usages: []ResourceUsage{
		{ResourceType: "application", ID: "a1", DisplayName: "App 1", BehaviorOnDelete: BehaviorFallback},
	}})
...
func TestGetUsagesNoProvidersReturnsEmpty(t *testing.T) {
	reg := NewRegistry()
