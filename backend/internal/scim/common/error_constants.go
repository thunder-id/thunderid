...
var (
	...
	// prefix that is neither the core User schema URN nor a well-formed
	...
	// ErrorEnterpriseSchemaNotSupported is returned when the Enterprise User schema
	// extension is supplied for a user type other than the designated core user type.
	ErrorEnterpriseSchemaNotSupported = tidcommon.ServiceError{
		Type: tidcommon.ClientErrorType,
		Code: "SCIM-1035",
		Error: tidcommon.I18nMessage{
			Key:          "error.scim.enterprise_schema_not_supported",
			DefaultValue: "Enterprise schema extension not supported",
		},
		ErrorDescription: tidcommon.I18nMessage{
			Key:          "error.scim.enterprise_schema_not_supported_description",
			DefaultValue: "The enterprise schema extension is only supported for the designated core user type",
		},
	}

	// ErrorUndeclaredEnterpriseSchemaObject is returned when the request body
	// contains an enterprise extension object key that is not declared in "schemas".
	ErrorUndeclaredEnterpriseSchemaObject = tidcommon.ServiceError{
		Type: tidcommon.ClientErrorType,
		Code: "SCIM-1036",
		Error: tidcommon.I18nMessage{
			Key:          "error.scim.undeclared_enterprise_schema_object",
			DefaultValue: "Undeclared enterprise schema object",
		},
		ErrorDescription: tidcommon.I18nMessage{
			Key: "error.scim.undeclared_enterprise_schema_object_description",
			DefaultValue: "The request body includes an enterprise extension object whose schema URN " +
				"is not declared in the schemas array",
		},
	}

	// errorUnrecognizedAttribute is returned when the "attributes" or
	// "excludedAttributes" parameter names an attribute path qualified with a
	// recognized core or enterprise schema URN, but whose attribute name is not
	// part of that schema.
	errorUnrecognizedAttribute = tidcommon.ServiceError{
		Type: tidcommon.ClientErrorType,
		Code: "SCIM-1037",
		Error: tidcommon.I18nMessage{
			Key:          "error.scim.unrecognized_attribute",
			DefaultValue: "Unrecognized attribute",
		},
		ErrorDescription: tidcommon.I18nMessage{
			Key:          "error.scim.unrecognized_attribute_description",
			DefaultValue: "The attribute name is not part of the schema named by its URN prefix",
		},
	}

	// errorSubAttributePathNotSupportedForProjection is returned when the "attributes" or
	// "excludedAttributes" query/body parameter (RFC 7644 §3.9 response-attribute
	// projection) names a dotted sub-attribute path (e.g. "name.familyName"). Only
	// root-level attribute paths are supported for projection, since a custom schema's
	// internal storage shape cannot be predicted generically. This restriction is scoped
	// to projection only — dotted sub-attribute paths remain supported in "filter" (see
	// scim.KindSubAttr / scim.KindMultiComplexPart in attr_rules.go).
	errorSubAttributePathNotSupportedForProjection = tidcommon.ServiceError{
		Type: tidcommon.ClientErrorType,
		Code: "SCIM-1038",
		Error: tidcommon.I18nMessage{
			Key:          "error.scim.sub_attribute_path_not_supported_for_projection",
			DefaultValue: "Sub-attribute path not supported for attribute projection",
		},
		ErrorDescription: tidcommon.I18nMessage{
			Key: "error.scim.sub_attribute_path_not_supported_for_projection_description",
			DefaultValue: "Dotted sub-attribute paths are not supported in \"attributes\"/\"excludedAttributes\"; " +
				"only root-level attribute paths are accepted for response projection",
		},
	}

	// ErrorCoreSchemaNotSupported is returned when the core User schema is supplied
	// for a user type other than the designated core user type.
	ErrorCoreSchemaNotSupported = tidcommon.ServiceError{
		Type: tidcommon.ClientErrorType,
		Code: "SCIM-1039",
		Error: tidcommon.I18nMessage{
			Key:          "error.scim.core_schema_not_supported",
			DefaultValue: "Core User schema not supported",
		},
		ErrorDescription: tidcommon.I18nMessage{
			Key:          "error.scim.core_schema_not_supported_description",
			DefaultValue: "The core User schema is only supported for the designated core user type",
		},
	}

	// ErrorNoTarget is returned when a PATCH "remove" operation has no "path" (RFC 7644 §3.5.2.2).
	ErrorNoTarget = tidcommon.ServiceError{
		Type: tidcommon.ClientErrorType,
		Code: "SCIM-1040",
		Error: tidcommon.I18nMessage{
			Key:          "error.scim.no_target",
			DefaultValue: "No target for patch operation",
		},
		ErrorDescription: tidcommon.I18nMessage{
			Key:          "error.scim.no_target_description",
			DefaultValue: `A "remove" operation requires a "path" naming the attribute to remove`,
		},
	}
)

// NewMissingRequiredAttributesError builds a SCIM-1017 (schema validation failed)
...
// core fields and the custom extension object.
...
// newInvalidFilterSyntaxError builds a SCIM-1032 error whose detail names the
...
// NewCustomAttributeRequiresURNError builds a SCIM-1033 error whose detail names the
...
// NewUnrecognizedSchemaURNError builds a SCIM-1034 error whose detail names the
...
// NewUnrecognizedAttributeError builds a SCIM-1037 error whose detail names the
// specific attribute path whose schema URN is recognized but whose attribute
// name does not belong to that schema.
func NewUnrecognizedAttributeError(attr string) *tidcommon.ServiceError {
	svcErr := errorUnrecognizedAttribute
	svcErr.ErrorDescription = tidcommon.I18nMessage{
		Key: errorUnrecognizedAttribute.ErrorDescription.Key,
		DefaultValue: fmt.Sprintf(
			"attribute %q is not a recognized attribute of the schema named by its URN prefix", attr),
	}
	return &svcErr
}

// NewSubAttrProjectionError builds a SCIM-1038 error whose detail names the specific
// dotted sub-attribute path rejected from "attributes"/"excludedAttributes".
func NewSubAttrProjectionError(attr string) *tidcommon.ServiceError {
	svcErr := errorSubAttributePathNotSupportedForProjection
	svcErr.ErrorDescription = tidcommon.I18nMessage{
		Key: errorSubAttributePathNotSupportedForProjection.ErrorDescription.Key,
		DefaultValue: fmt.Sprintf(
			"attribute %q is a dotted sub-attribute path, which is not supported for response projection; "+
				"use the root attribute name", attr),
	}
	return &svcErr
}

// BuildUserTypeErrorToSCIM translates an entitytype service error into a SCIM
...
func BuildUserTypeErrorToSCIM(svcErr *tidcommon.ServiceError) *tidcommon.ServiceError {
	if svcErr == nil {
		return nil
	}
