...
func (r *entityTypeResolution) resolve(ctx *providers.NodeContext, category entitytype.TypeCategory,
	...
	if selected, ok := ctx.UserInputs[typeInputKey(category)]; ok && selected != "" {
...
// typeInputKey returns the input identifier a flow declares for the category's type. Flow
// definitions name these, unlike categoryTypeKey, so the two cannot be merged.
func typeInputKey(category entitytype.TypeCategory) string {
