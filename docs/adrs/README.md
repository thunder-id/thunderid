...
To change a decision, write a new ADR that declares what it replaces.
If the new record is ADR-0014 and it replaces ADR-0007, then ADR-0014 carries:

```yaml
supersedes: "ADR-0007"
```

and exactly one line changes in ADR-0007:
...
The lint rejects a pair that points only one way, and a reference to a record that does not exist.

The status field carries the identifier only, not a link. This is a MADR 4.0 convention.

A record that is deprecated rather than replaced says why in `## More Information`, because there is no successor record to carry that reasoning.
...
Copy one of the two templates to `NNNN-title-with-dashes.md`.

* [`adr-template.md`](adr-template.md) is the full MADR template, with decision drivers, consequences, confirmation, and a per-option comparison. Use it when the options need arguing.
* [`adr-template-minimal.md`](adr-template-minimal.md) carries the mandatory sections and nothing else beyond `## More Information`. Use it when the decision is small and the reasoning fits in a paragraph.

Reaching for the minimal template is better than reaching for the full one and leaving half of it as placeholder text.
