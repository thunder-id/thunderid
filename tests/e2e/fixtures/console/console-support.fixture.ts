 ...
 *
 * - `usersApi` / `userTypesApi` / `applicationsApi`: the shared API helpers bound to the
 *   test's request context. `beforeAll`/`afterAll` cannot take test-scoped fixtures, so they
 *   construct `new UsersApi(request)` (etc.) themselves - same class, so no logic lives in
 *   two places.
