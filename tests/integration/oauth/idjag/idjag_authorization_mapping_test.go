...
/*
ID-JAG consumption-leg AuthorizationRuleMapping integration tests.

These exercise the connection's authorizationMapping.rules section on the ID-JAG consumption leg (the
...
IDJAGConsumptionTestSuite, so its connection's authorizationMapping.rules can be rewritten per scenario
...
const (
	...
	idjagAuthzAppName        = "IDJAGAuthorizationRuleMappingApp"
...
// IDJAGAuthorizationRuleMappingTestSuite exercises the ID-JAG consumption leg's AuthorizationRuleMapping
...
type IDJAGAuthorizationRuleMappingTestSuite struct {
	suite.Suite
	client *http.Client
...
func TestIDJAGAuthorizationRuleMappingTestSuite(t *testing.T) {
	suite.Run(t, new(IDJAGAuthorizationRuleMappingTestSuite))
}

func (ts *IDJAGAuthorizationRuleMappingTestSuite) SetupSuite() {
...
func (ts *IDJAGAuthorizationRuleMappingTestSuite) TearDownSuite() {
	if ts.appID != "" {
...
func (ts *IDJAGAuthorizationRuleMappingTestSuite) createApp() string {
	ts.T().Helper()
	app := map[string]any{
...
func (ts *IDJAGAuthorizationRuleMappingTestSuite) applyAuthorizationRuleMapping(
...
func (ts *IDJAGAuthorizationRuleMappingTestSuite) applyAuthorizationRuleMappingRules(
...
// type).
func (ts *IDJAGAuthorizationRuleMappingTestSuite) applyAuthorizationRuleMappingRulesAndDelimiter(
...
func (ts *IDJAGAuthorizationRuleMappingTestSuite) nextJTI() string {
	ts.jtiCounter++
	return fmt.Sprintf("idjag-authz-jti-%d-%d", ts.jtiCounter, time.Now().UnixNano())
}

// buildAssertion signs an ID-JAG assertion from the mock issuer. The assertion's own scope claim
// covers every scope requested elsewhere in this file ("read write"), since the granted scopes are
// always bounded by it (draft-ietf-oauth-identity-assertion-authz-grant §4.4.1): a mapped scenario
// still has to stay within it to prove mapping resolves the requested scope, which
// TestIDJAGConsumption_MappedRoleCannotGrantScopeBeyondAssertionScope tests directly with a
// narrower, scenario-specific scope claim.
func (ts *IDJAGAuthorizationRuleMappingTestSuite) buildAssertion(customClaims map[string]interface{}) string {
	ts.T().Helper()
...
func (ts *IDJAGAuthorizationRuleMappingTestSuite) consumeAssertion(assertion, scope, resource string) (
...
// TestIDJAGConsumption_MappedRoleGrantsScopeWithinAssertionScope covers a mapped role (no local
// assignees) resolved from the assertion's claims: the mapped role only grants read, so read is
// authorized and write is not, independent of the assertion's own scope claim.
func (ts *IDJAGAuthorizationRuleMappingTestSuite) TestIDJAGConsumption_MappedRoleGrantsScopeWithinAssertionScope() {
	...
	resp, status := ts.consumeAssertion(assertion, "read write", idjagAuthzRSIdentifier)
...
// TestIDJAGConsumption_MappedRoleCannotGrantScopeBeyondAssertionScope covers
// draft-ietf-oauth-identity-assertion-authz-grant §4.4.1 ("granted scopes MAY be a subset of the
// scopes in the ID-JAG"): a role mapped from the assertion's own claims would grant write, but the
// assertion's own scope claim here carries only read, so write must not be authorized regardless.
func (ts *IDJAGAuthorizationRuleMappingTestSuite) TestIDJAGConsumption_MappedRoleCannotGrantScopeBeyondAssertionScope() {
	...
	header := map[string]interface{}{"typ": "oauth-id-jag+jwt", "alg": "RS256", "kid": "oidc-key-1"}
	now := time.Now()
	assertion, err := ts.mockIssuer.SignJWT(header, map[string]interface{}{
		"iss":        ts.mockIssuer.GetURL(),
		"sub":        "idjag-authz-mapping-subject",
		"aud":        idjagAuthzAudience,
		"client_id":  idjagAuthzClientID,
	...
	resp, status := ts.consumeAssertion(assertion, "read write", idjagAuthzRSIdentifier)
	ts.Require().Equal(http.StatusOK, status, "error=%s description=%s", resp.Error, resp.ErrorDescription)
	ts.NotContains(strings.Fields(resp.Scope), "write",
		"write is absent from the assertion's own scope claim, so the mapped group's role must not grant it")
}
...
func (ts *IDJAGAuthorizationRuleMappingTestSuite) TestIDJAGConsumption_MappedGroupGrantsScope() {
	ts.applyAuthorizationRuleMapping("department", "", map[string][]testutils.AuthorizationTarget{
		"platform": {{Type: testutils.AuthorizationTargetGroup, ID: ts.mappedGroupID}},
	})
	...
	ts.NotContains(scopes, "read", "the mapped group's role only grants write")
}
...
func (ts *IDJAGAuthorizationRuleMappingTestSuite) TestIDJAGConsumption_MappedPermissionGrantsDirectly() {
...
// scope.
func (ts *IDJAGAuthorizationRuleMappingTestSuite) TestIDJAGConsumption_UnmappedValueGrantsNoScope() {
	ts.applyAuthorizationRuleMapping("groups", "", map[string][]testutils.AuthorizationTarget{
		"idjag-admins": {{Type: testutils.AuthorizationTargetRole, ID: ts.mappedRoleID}},
	})
...
func (ts *IDJAGAuthorizationRuleMappingTestSuite) TestIDJAGConsumption_ListValuedClaimGrantsRole() {
	ts.applyAuthorizationRuleMapping("groups", "", map[string][]testutils.AuthorizationTarget{
		"idjag-admins": {{Type: testutils.AuthorizationTargetRole, ID: ts.mappedRoleID}},
	})
...
func (ts *IDJAGAuthorizationRuleMappingTestSuite) TestIDJAGConsumption_NotEqualsOperatorMatchesElementInListClaim() {
...
func (ts *IDJAGAuthorizationRuleMappingTestSuite) TestIDJAGConsumption_NotIncludesWithholdsWhenExcludedValuePresentInListClaim() {
...
func (ts *IDJAGAuthorizationRuleMappingTestSuite) TestIDJAGConsumption_IncludesOperatorTestsSetMembershipInListClaim() {
	ts.applyAuthorizationRuleMappingRules("groups", testutils.AuthorizationValueTypeArray, []testutils.AuthorizationRule{
		{
...
func (ts *IDJAGAuthorizationRuleMappingTestSuite) TestIDJAGConsumption_SpaceDelimitedClaimGrantsRole() {
...
func (ts *IDJAGAuthorizationRuleMappingTestSuite) TestIDJAGConsumption_PermissionTargetScopedToItsOwnResourceServer() {
	ts.applyAuthorizationRuleMapping("groups", "", map[string][]testutils.AuthorizationTarget{
		"idjag-admins": {{
			Type: testutils.AuthorizationTargetPermission, ResourceServerID: ts.resourceServerID, Permission: "read",
		}},
