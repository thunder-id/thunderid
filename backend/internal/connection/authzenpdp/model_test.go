...
func TestConnectionRequestServerDefaultsAndOverrides(t *testing.T) {
	cfg := &config.GetServerRuntime().Config.AuthZENPDP
	original := *cfg
	t.Cleanup(func() { *cfg = original })
	retries := 4
	cfg.TimeoutMS, cfg.RetryCount = 1200, &retries
	var inherited, explicit ConnectionRequest
	require.NoError(t, json.Unmarshal([]byte(`{}`), &inherited))
	require.Equal(t, 1200, inherited.TimeoutMS)
	require.Equal(t, 4, inherited.RetryCount)
	require.NoError(t, json.Unmarshal([]byte(`{"timeoutMs":45000,"retryCount":0}`), &explicit))
	connection := FromRequest(explicit)
	require.Equal(t, 45000, connection.TimeoutMS)
	require.Zero(t, connection.RetryCount)
	data, err := json.Marshal(explicit)
	require.NoError(t, err)
	require.NoError(t, json.Unmarshal(data, &inherited))
	require.Zero(t, inherited.RetryCount)
}
