...
func (f *entityFileBasedStore) GetEntityListCount(ctx context.Context, category string,
	...
	for _, resource := range resources {
		...
		combined := mergeJSONObjects(resource.Entity.Attributes, resource.Entity.SystemAttributes)
		if matchesFilters(combined, filters) && matchesSearch(combined, search) {
			count++
		}
	}
	return count, nil
...
// matchesSearch reports whether the given attributes satisfy the SCIM-style filter group.
...
func matchesSearch(attributes json.RawMessage, g *tidcommon.FilterGroup) bool {
	if !hasFilterGroup(g) {
		return true
	}
