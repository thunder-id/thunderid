...
Cross-Origin Resource Sharing (CORS) controls which origins can make requests to <ProductName /> from a browser. Allowed origins are managed through the server-config `cors` section. No origins are allowed by default; in production, you must explicitly list every origin that legitimately needs access.

Provide your allowed origins as a declarative baseline: create `config/resources/server_configs/cors.yaml` and enable the server-config store by setting `server_config.store: composite` in `deployment.yaml` so it loads at boot (the default, `mutable`, does not load declarative resources):
...
You can also add or change origins at runtime — without a restart — with `PUT /server-config/cors` (the runtime value is merged with this baseline).
...
- If your domain follows a consistent pattern, you can use a regular expression:
  ...
  Regex patterns must be fully anchored — start with `^` and end with `$`. <ProductName /> logs a warning at startup for any pattern that is not fully anchored, because unanchored patterns match more origins than intended.
