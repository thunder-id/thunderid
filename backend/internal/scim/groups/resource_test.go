...
func TestGroupVersionState(t *testing.T) {
	t.Run("deterministic regardless of member order", func(t *testing.T) {
		...
		require.Equal(t, scim.GenerateVersion(groupVersionState(g1)), scim.GenerateVersion(groupVersionState(g2)))
	})
	...
	t.Run("group name change changes version state", func(t *testing.T) {
		...
		require.NotEqual(t, scim.GenerateVersion(groupVersionState(g1)), scim.GenerateVersion(groupVersionState(g2)))
	})
	...
	t.Run("empty and nil members produce equivalent state", func(t *testing.T) {
		...
		require.Equal(t, scim.GenerateVersion(groupVersionState(gNil)), scim.GenerateVersion(groupVersionState(gEmpty)))
	})
}
