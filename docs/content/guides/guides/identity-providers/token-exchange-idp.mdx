...
This uses [RFC 8693 Token Exchange](https://datatracker.ietf.org/doc/html/rfc8693). For general information about the token exchange grant type, see [Token Exchange](../protocols/oauth-oidc/token-exchange.mdx).
...
- <ProductName /> is running. See [Get Started](../../getting-started/get-thunderid.mdx).
...
```bash
curl -X POST https://{{productSlug}}.example.com/identity-providers \
...
```bash
curl -X POST https://{{productSlug}}.example.com/applications \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
- [Token Exchange](../protocols/oauth-oidc/token-exchange.mdx) — Learn about the RFC 8693 token exchange grant type in <ProductName />.
