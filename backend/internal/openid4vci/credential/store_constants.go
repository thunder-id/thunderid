...
var (
	queryCreateConfiguration = dbmodel.DBQuery{
		...
		Query: `INSERT INTO "CREDENTIAL_CONFIGURATION" ` +
			`(ID, HANDLE, OU_ID, FORMAT, VCT, CLAIMS, DISPLAY, VALIDITY_SECONDS, DEPLOYMENT_ID) ` +
			`VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
	}
	queryGetConfigurationByID = dbmodel.DBQuery{
		ID: "OVCIQ-CC_MGT-02",
		Query: `SELECT ID, HANDLE, OU_ID, FORMAT, VCT, CLAIMS, DISPLAY, VALIDITY_SECONDS ` +
	...
	queryListConfigurationSummaries = dbmodel.DBQuery{
		ID: "OVCIQ-CC_MGT-07",
		Query: `SELECT ID, HANDLE, OU_ID, FORMAT, VCT, DISPLAY ` +
			`FROM "CREDENTIAL_CONFIGURATION" WHERE DEPLOYMENT_ID = $1`,
	}
	queryUpdateConfiguration = dbmodel.DBQuery{
		ID: "OVCIQ-CC_MGT-05",
		Query: `UPDATE "CREDENTIAL_CONFIGURATION" SET HANDLE = $2, OU_ID = $3, FORMAT = $4, ` +
			`VCT = $5, CLAIMS = $6, DISPLAY = $7, VALIDITY_SECONDS = $8, UPDATED_AT = CURRENT_TIMESTAMP ` +
			`WHERE ID = $1 AND DEPLOYMENT_ID = $9`,
	}
