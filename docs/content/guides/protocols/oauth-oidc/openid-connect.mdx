...
| `sid` | Identifier of the SSO session the authentication belongs to. Applications that share one session receive the same value, and it stays the same across refresh. Omitted when the login flow established no session |
