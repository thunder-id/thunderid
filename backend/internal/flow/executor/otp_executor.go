...
// token keyed to the userID, and forwards the plaintext OTP via ForwardedData so that a
// downstream sender executor (SMSExecutor, EmailExecutor, etc.) can render it into a template
// without persisting it for the life of the flow session.
...
func (e *otpExecutor) executeGenerate(ctx *core.NodeContext,
	...
	logger.Info(ctx.Context, "OTP generate started",
		log.String("flowType", string(ctx.FlowType)),
		log.Int("nodeInputCount", len(ctx.NodeInputs)),
		log.Int("userInputCount", len(ctx.UserInputs)),
		log.Bool("hasUserIDInRuntime", ctx.RuntimeData[userAttributeUserID] != ""),
	)
	...
	logger.Info(ctx.Context, "OTP generate: resolveUserID result",
		log.String("resolvedUserID", userID),
	...
	if recipient == "" && execResp.Status == common.ExecFailure {
		if ctx.FlowType != common.FlowTypeRegistration {
			logger.Info(ctx.Context, "OTP generate: user not found, non-registration flow — aborting",
				log.String("errorCode", func() string {
		...
		logger.Info(ctx.Context, "OTP generate: registration flow — resolved destination",
			log.String("destAttr", destAttr),
			log.Bool("hasDestValue", destValue != ""),
		)
		if destValue == "" {
			logger.Info(ctx.Context, "OTP generate: no destination value in registration flow, prompting for input")
	...
	if execResp.Status == common.ExecUserInputRequired {
		logger.Info(ctx.Context, "OTP generate: user input required, returning inputs",
			log.Int("inputCount", len(execResp.Inputs)),
		)
		return execResp, nil
	}

	logger.Info(ctx.Context, "OTP generate: calling GenerateOTP",
		log.String("recipientAttr", recipientAttr),
	)
	...
	execResp.ForwardedData[common.ForwardedDataKeyTemplateData] = map[string]interface{}{
		common.RuntimeKeyOTPValue: otpValue,
		"expiryMinutes":           strconv.FormatInt(expirySeconds/60, 10),
	}
	execResp.Status = common.ExecComplete

	logger.Info(ctx.Context, "OTP generated successfully")
	return execResp, nil
}
...
func (e *otpExecutor) resolveUserID(ctx *core.NodeContext,
	...
	if userID := ctx.RuntimeData[userAttributeUserID]; userID != "" {
		logger.Info(ctx.Context, "OTP generate: userID resolved from RuntimeData")
		return userID, nil
	}
	...
	attrKeys := make([]string, 0, len(searchAttrs))
	for k := range searchAttrs {
		attrKeys = append(attrKeys, k)
	}
	logger.Info(ctx.Context, "OTP generate: buildSearchAttributes result",
		log.Int("attrCount", len(searchAttrs)),
		log.String("attrKeys", fmt.Sprintf("%v", attrKeys)),
	)

	if len(searchAttrs) == 0 {
		logger.Info(ctx.Context, "OTP generate: no searchable inputs found — prompting for input",
			log.Int("nodeInputCount", len(ctx.NodeInputs)),
			log.Int("userInputCount", len(ctx.UserInputs)),
			log.Int("runtimeDataCount", len(ctx.RuntimeData)),
		)
		execResp.Status = common.ExecUserInputRequired
	...
	if providerErr != nil {
		if providerErr.Code == entityprovider.ErrorCodeEntityNotFound {
			logger.Info(ctx.Context, "OTP generate: user not found for provided search attrs",
				log.String("attrKeys", fmt.Sprintf("%v", attrKeys)),
			)
	...
	if identifiedUserID == nil || *identifiedUserID == "" {
		logger.Info(ctx.Context, "OTP generate: IdentifyEntity returned empty userID")
		execResp.Status = common.ExecFailure
		execResp.Error = &ErrUserNotFound
		return "", nil
	}

	logger.Info(ctx.Context, "OTP generate: user identified successfully")
...
func (e *otpExecutor) buildSearchAttributes(ctx *core.NodeContext) map[string]interface{} {
	...
	logger.Info(ctx.Context, "OTP generate: buildSearchAttributes scanning inputs",
		log.Int("inputCount", len(inputs)),
		log.Bool("usedFallbackInputs", usedFallback),
	)

	for _, input := range inputs {
		inUserInputs := func() bool { v, ok := ctx.UserInputs[input.Identifier]; return ok && v != "" }()
		inRuntimeData := func() bool { v, ok := ctx.RuntimeData[input.Identifier]; return ok && v != "" }()
		inForwardedData := func() bool {
			v, ok := ctx.ForwardedData[input.Identifier]
			if !ok {
				return false
			}
			s, isStr := v.(string)
		...
		skipped := input.Identifier == userInputOTP || !isSearchableIdentifier(input.Identifier)

		logger.Info(ctx.Context, "OTP generate: input scan",
			log.String("identifier", input.Identifier),
			log.String("type", input.Type),
			log.Bool("skipped", skipped),
			log.Bool("foundInUserInputs", inUserInputs),
			log.Bool("foundInRuntimeData", inRuntimeData),
			log.Bool("foundInForwardedData", inForwardedData),
		)
		...
		if inUserInputs {
			attrs[input.Identifier] = ctx.UserInputs[input.Identifier]
			continue
		}
		if inRuntimeData {
			attrs[input.Identifier] = ctx.RuntimeData[input.Identifier]
			continue
		}
...
// getAuthenticatedUser verifies the OTP code directly against the OTP service.
func (e *otpExecutor) getAuthenticatedUser(ctx *core.NodeContext,
	...
	authnResult, svcErr := e.otpService.Authenticate(ctx.Context, sessionToken, providedOTP)
	if svcErr != nil {
		if svcErr.Code == otp.ErrorIncorrectOTP.Code ||
			svcErr.Code == otp.ErrorInvalidSessionToken.Code {
	...
	for key, value := range authnResult.AuthenticatedClaims {
...
// resolveOTPDestination returns the identifier name and its value from the first searchable,
// non-OTP declared NodeInput. Accepts any channel-appropriate type (PHONE_INPUT, EMAIL_INPUT, etc.).
// The value is looked up in UserInputs, RuntimeData, and ForwardedData.
...
func (e *otpExecutor) resolveOTPDestination(ctx *core.NodeContext) (attrName, attrValue string) {
	for _, input := range ctx.NodeInputs {
		if (input.Type != common.InputTypePhone && input.Type != common.InputTypeEmail) ||
			!isSearchableIdentifier(input.Identifier) {
			continue
		}
...
func (e *otpExecutor) getGenerateInputs(ctx *core.NodeContext) []common.Input {
...
func (e *otpExecutor) getMaxOTPAttempts(ctx *core.NodeContext) int {
