...
func (suite *CreateSecurityMiddlewareTestSuite) TestCreateSecurityMiddleware_MultipleInvocations() {
	...
	// An empty config leaves server.security.rest.audience unset, the REST gate's default.
	cfg := &config.Config{}

	handler1 := createSecurityMiddleware(context.Background(), suite.logger, cfg, suite.mux, suite.mockJWTService, nil)
	handler2 := createSecurityMiddleware(context.Background(), suite.logger, cfg, suite.mux, suite.mockJWTService, nil)
	handler3 := createSecurityMiddleware(context.Background(), suite.logger, cfg, suite.mux, suite.mockJWTService, nil)
...
func TestRegisterStaticFileHandlers(t *testing.T) {
	...
	// Both directories are created, so the assertion that the Gate is not served reflects a
	// deliberate choice here rather than a missing build artifact.
	...
	t.Run("registers the console handler", func(t *testing.T) {
		mux := http.NewServeMux()
	...
	// The Gate is the login application, and a Control Plane serves no login traffic. Its
	// directory exists in this fixture and is still not served.
	t.Run("does not register the gate handler", func(t *testing.T) {
		mux := http.NewServeMux()
		registerStaticFileHandlers(context.Background(), logger, mux, tmpDir)
		...
		mux.ServeHTTP(rr, req)
		assert.Equal(t, http.StatusNotFound, rr.Code,
			"the Control Plane must not serve the Gate application")
	})

	t.Run("serves console js files as application/javascript", func(t *testing.T) {
		requireWriteFile(t, filepath.Join(consoleDir, "app.js"), []byte("console.log('hello');"))
	...
	t.Run("serves console mjs files as application/javascript", func(t *testing.T) {
		requireWriteFile(t, filepath.Join(consoleDir, "app.mjs"), []byte("export default {};"))

		mux := http.NewServeMux()
		registerStaticFileHandlers(context.Background(), logger, mux, tmpDir)

		req := httptest.NewRequest(http.MethodGet, "/console/app.mjs", nil)
		rr := httptest.NewRecorder()
		mux.ServeHTTP(rr, req)
...
func TestAccessLogExcludePaths(t *testing.T) {
	// Only the console prefix is built in: the Control Plane serves no Gate application.
	assert.Equal(t, []string{"/console/"}, accessLogExcludePaths(nil))

	// Configured prefixes are appended after the built-in console prefix.
	assert.Equal(t, []string{"/console/", "/health/", "/metrics/"},
	...
	assert.Equal(t, []string{"/console/", "/health/"},
